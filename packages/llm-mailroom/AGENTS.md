...
PYTHONPATH=src python src/scripts/run_hf_pilot.py --real --per-class 1  # stratified docclass-merged v5 subset → Langfuse session pilot-hf-<stamp>
...
PYTHONPATH=src python src/scripts/sync_hf_ground_truth.py --real --push  # LLM-label the docclass-merged ground_truth config (needs OPENROUTER_API_KEY + HF_TOKEN); re-pins FULL_CORPUS_REVISION
PYTHONPATH=src python src/scripts/sync_hf_ground_truth.py --check   # network-free contract for the intent/subject_matter/keywords ground-truth labeler
PYTHONPATH=src python src/scripts/sync_hf_ground_truth.py --dry-run # derive purpose/gist labels for corporate_record/correspondence/insurance_claim → data/hf_gt/ preview CSV
PYTHONPATH=src python src/scripts/sync_hf_ground_truth.py --real --push  # LLM-label the docclass-merged ground_truth config (needs OPENROUTER_API_KEY + HF_TOKEN); re-pins FULL_CORPUS_REVISION
...
- Tests: `pytest -v` (whole suite), `pytesttest_agents/ -v`, `pytesttest_routing.py`, `-k "sorter"` for single-agent. Coverage via `--cov=src --cov-report=html`.
