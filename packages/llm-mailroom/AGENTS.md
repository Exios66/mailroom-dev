...
jupyter notebook notebooks/14_gmail_pilot.ipynb     # ONE-document corpus pilot: pick a mailroom-corpus doc (offline snapshot or live Hub), fire it through the agent mailbox (mock drop | real SMTP; FIRE interlock), watch the free triage lane, verify against the corpus ground truth, log data/pilot_runs/<stamp>/report.{json,md}
