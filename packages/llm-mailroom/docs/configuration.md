...
| `MAILROOM_GMAIL_TRIAGE` | No | `1` (with channel on) | Pre-pipeline intake triage (HUB-037): at watcher claim time the preliminary classification read runs on the registry's most capable OpenRouter model (`deepseek/deepseek-v4-pro`, advisory) — primary class, subclass, confidence, gist, keywords — riding `intake.triage` into the manifest and the echo. Advisory — never influences the pipeline's own classification. Fails soft; set `0` to disable |
