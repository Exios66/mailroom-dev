...
| Grouping metrics | scaffolded | §14A verified baseline (19/350 heuristic thread rows; header threads structurally absent) + `mailroom_eda.bundles` synthetic families; pairwise precision/recall (§25) is computable once a published revision carries bundle assignments (§84 decision). **PUBLISHED 2026-09-02**: the `bundles` config (50 rows) ships the assignments; pairwise metrics still need pipeline runs over them. |
...
| Stream-level evaluation | blocked-on-runs | **UNBLOCKED 2026-09-02**: the `streams` config ships `simulation_run_id` + `sequence_position` + distractors (§27–29/§48 — `RUN-SIM-001`, 62 rows over the 10 bundle matters). The §84A chain is complete: §14A decision → P2 grouping → publication → simulation. What remains is pipeline runs over the stream (the stream itself is real data now). |
