...
verified — 12 new [`tests/test_env_utils.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/tests/test_env_utils.py) tests + all runner smoke
...
Contract-specialist v1..v16 archive shipped in [commit `e0f758e`](https://github.com/Exios66/llm-entity-extraction/commit/e0f758e) (CHANGELOG `[Unreleased]` Changed in the same commit; 384 tests green): [`src/prompts_archive.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/src/prompts_archive.py) holds the FROZEN pre-documentation lineage (v1..v16, ~1,000 lines / ~72 KB), [`src/prompts.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/src/prompts.py) imports it back — **file 227 KB → 155 KB (−32%), all 32 version strings byte-identical (verified against git HEAD), every version key resolvable** (`get_prompt`, `PROMPT_VERSIONS`, manifests, Langfuse prompt syncs). Documented frontier lineage (v17..v32) with data-backed banners stays in `prompts.py`. Archive invariant pinned by `test_contracts_archive_preserves_identity_and_version_keys` + `test_contracts_archive_chain_heads_resolve`. Card archived.
...
Run sink swapped to **Langfuse + LangSmith**; Braintrust experiment/span logging is now OFF by default — no subscription upgrade needed. `BRAINTRUST_LOGGING=disabled` (default in `.env`/[`.env.example`](https://github.com/Exios66/llm-entity-extraction/blob/main/.env.example)): the four `run_*_eval.py` runners (subtype/extraction/chained/classification) consult it via [`src/braintrust_logging.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/src/braintrust_logging.py) and, when disabled, skip `setup_langchain` + `braintrust.Eval` entirely, running the SAME local scoring loop through `src/eval_shims.py::run_local_eval` (thread pool + manifest resume + repo experiment log, `tracing_backend="none"` + `langsmith` meta) — every surface incl. vision classification + chunked extraction runs with ZERO Braintrust quota; opt back in per run with `BRAINTRUST_LOGGING=enabled`. **`run_langfuse_*_eval.py` are now the documented PRIMARY path** (per-document Langfuse traces + numeric scores in llm-dojo; every LLM call auto-traces to the LangSmith `llm-mailroom` project; chunked extraction supported). Verified live: 1-doc subtype pilot through the disabled path → `tracing_backend=none`, `langsmith=True`, full RunnableSequence→ChatOpenAI trace tree captured in LangSmith, and no Braintrust 400s (previously every log batch was dropped on `num_log_bytes_calendar_months`). Docs flipped (AGENTS.md intro + cheatsheet, README, wiki/Eval-Runners); 365 tests green (+5 gate unit tests, +1 disabled-path smoke test). Card archived.
...
LegalBench **hearsay** task
  ...
  [`data/legalbench_classes.jsonl`](https://github.com/Exios66/llm-entity-extraction/blob/main/data/legalbench_classes.jsonl) was never written. **Shipped:** (1) the
...
prompt-engineer agent shipped
  ...
  [`AGENTS.md`](https://github.com/Exios66/llm-entity-extraction/blob/main/AGENTS.md) "Agents (this repo)" section documents it alongside
...
The **prompt-engineer agent**
  ...
  rules, and the version-key identity invariant. [`AGENTS.md`](https://github.com/Exios66/llm-entity-extraction/blob/main/AGENTS.md) gains the
...
The
  ...
  signed_mean + per-field + n_docs, date/duration/money_n_pairs — [`src/metrics.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/src/metrics.py),
  `parse_money` alias, 4 new tests); (2) **dedicated run-level diagnostics
  renderer** in [`src/experiment_log.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/src/experiment_log.py) (`_diagnostics_lines`: list quality,
  ...
  diagnostics card** ([`docs/assets/site.js`](https://github.com/Exios66/llm-entity-extraction/blob/main/docs/assets/site.js) `diagnosticsCard()` + styles);
...
**v0.18.0 shipped**: changelog conversion (Unreleased → v0.18.0, 6 entries), pyproject 0.18.0, tag `v0.18.0` + GitHub release with changelog notes; board swept (KANBAN-004/017/018/019/020 archived under v0.18.0, open cards 005/006/008/009/011/013 re-targeted v0.19.0). **Root-cause fix in [`scripts/release.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/scripts/release.py)**: the conversion inserted the new release section under the `[Unreleased]` header but never removed the old body — every converted section shipped duplicated (v0.15.0/v0.17.0/v0.18.0 all had to be deduped by hand). `release_changelog` now replaces the WHOLE Unreleased section and the note lives inside the new release header (no more phantom unreleased bullets); regression test `test_release_changelog_does_not_duplicate_entries` added (348 tests green). llm-mailroom mirror synced (`19c8f63`).
...
Slides problems/fixes decks claimed `in_progress` (board-only, docs-only): four new decks in `docs/slides/` — 08/09 = sorter problems then fixes, 10/11 = contracts-specialist problems then fixes (same framing, one doc per side per agent) — built from the changelog iteration records (v0.15.0/v0.16.0/v0.17.0), [`V16_PROPOSITION.md`](https://github.com/Exios66/llm-entity-extraction/blob/main/V16_PROPOSITION.md) summaries, and the KANBAN-016/017 arms. README index updated. No changelog entry (docs-only).
...
Extraction
  ...
  **Concurrent-edit note for future agents:** mid-session, a parallel edit
  landed in the same files (a diagnostics renderer in [`src/experiment_log.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/src/experiment_log.py),
  money-MAE + span-count-drift metrics in [`src/metrics.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/src/metrics.py), `parse_money`
  alias, [`tests/test_experiment_log.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/tests/test_experiment_log.py), site display + regenerated
...
Full-corpus CUAD EDA landed in
  ...
  same commit, 306 tests green): [`data/eda/report.md`](https://github.com/Exios66/llm-entity-extraction/blob/main/data/eda/report.md) + `findings.md` +
  `figures/01`–`10`, driven by [`scripts/eda/explore_cuad.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/scripts/eda/explore_cuad.py) (reproducible:
...
Full-corpus EDA
  (510-contract CUAD) in progress: [`scripts/eda/explore_cuad.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/scripts/eda/explore_cuad.py) rewritten
  ...
  shares; co-occurrence; redaction scan) → outputs [`data/eda/report.md`](https://github.com/Exios66/llm-entity-extraction/blob/main/data/eda/report.md),
  [`data/eda/findings.md`](https://github.com/Exios66/llm-entity-extraction/blob/main/data/eda/findings.md), `figures/01`–`10`. Key numbers: median 33,425
...
Board
  ...
  (`[#NNN](https://github.com/Exios66/llm-entity-extraction/blob/main/url)`) — one card = one issue, card↔issue status never disagrees;
...
In-progress semantics
  ...
  tree right now ([`src/prompts.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/src/prompts.py), [`tests/test_prompts.py`](https://github.com/Exios66/llm-entity-extraction/blob/main/tests/test_prompts.py)), which made the
...
Board sweep: sorter_v7
  ...
  [`docs/data/board.json`](https://github.com/Exios66/llm-entity-extraction/blob/main/docs/data/board.json)) — links to each card's GitHub issue.
...
Board created and
  ...
  tree (KANBAN-007). Open questions from [`V16_PROPOSITION.md`](https://github.com/Exios66/llm-entity-extraction/blob/main/V16_PROPOSITION.md) promoted to
