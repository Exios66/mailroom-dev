...
| KANBAN-029 | — (board-only) | `in_progress` | **effective_date rule_contradiction repair (v32)** — full-corpus diagnosis on the v31@510 reasoning-trace corpus (champion 0.8737): `effective_date` 0.8577 @510, 51/509 docs (10%) at 0.0. Root cause = `rule_contradiction`: the v12-era "defined term wins" tie-break vs CUAD GT holding the Agreement/Execution date as answers[0] in 493/493 docs. **v32 = v31.replace(effective_date rule)**: Agreement/EXECUTION date wins whenever stated; defined "Effective Date" term fallback only when no execution date appears; never null when a stated date is visible. **Pair completed: candidate 00:02 DEGRADED (52/509 transient errors, 457 scored) vs control 20:26 (champion re-measured 0.8737, 5 errors). Intersection signal (454 rows both-ok): paired +0.0115, CI [+0.0017, +0.0215], P=0.0115; effective_date +0.0299 (24 recovered / 6 regressed). The 6 regressions = NEW rule-driven over-fire cluster (never-null clause on source-metadata date / blank-day template / "date first above written"; 2 signature-boundary shifts; 1 execution-wins-over-effective-as-of) → banked as the v33 lesson.** Degraded candidate → clean rerun REQUIRED for release verdict: `qwen3.7-flash_contracts_specialist_v32_extraction_langfuse_510_full_clean` running (fresh manifest `data/manifests/v32_510_chunked_full_clean.jsonl`, full-510 chunked, seed 42, temp 0.1, reasoning none). | prompt-engineer (2026-08-15) | v0.19.0 | `src/prompts.py` CONTRACTS_SPECIALIST_PROMPT_V32; `tests/test_prompts.py::test_contracts_v32_effective_date_convention_fix`; memo `memos/contracts_specialist_v32.md` |
