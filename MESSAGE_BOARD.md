...
| KANBAN-029 | — (board-only) | `in_progress` | **effective_date rule_contradiction repair (v32)** — full-corpus diagnosis on the v31@510 reasoning-trace corpus (champion 0.8737): `effective_date` 0.8577 @510, 51/509 docs (10%) at 0.0. Root cause = `rule_contradiction`: the v12-era "defined term wins" tie-break vs CUAD GT holding the Agreement/Execution date as answers[0] in 493/493 docs; on the 26 differing-date docs (Monsanto AG 2017-08-31/EF 1998-09-30, IMAGEWARE, PACIRA, ArcGroup, UnionDental, NETGEAR, …) → 6 at 0.0 + 14 partial; plus 23 null-when-date-present docs (GULFSOUTH "executed as of the 14th day of December, 1997" → null). **v32 = v31.replace(effective_date rule)**: the Agreement/EXECUTION date wins whenever one is stated; a defined "Effective Date" term is fallback only when no execution date appears; never null when a stated date is visible. Ceiling +0.0142 composite vs the ±0.011 band (v31 CI half-width [0.8625, 0.8852]) → full-510 A/B required (the 26 differing-date docs are absent from the 50-doc and sample5 surfaces). Runs reserved: `qwen3.7-flash_contracts_specialist_v32_extraction_langfuse_510_full` (candidate) + `qwen3.7-flash_contracts_specialist_v31_extraction_langfuse_510_rerun` (noise-floor control) — full-510 chunked, seed 42, temp 0.1, reasoning none, manifest `data/manifests/v31_510_chunked_full.jsonl`. | prompt-engineer (2026-08-15) | v0.19.0 | `src/prompts.py` CONTRACTS_SPECIALIST_PROMPT_V32; `tests/test_prompts.py::test_contracts_v32_effective_date_convention_fix`; memo `memos/contracts_specialist_v32.md` |
