...
|---|---|---|---|---|---|---|
| KANBAN-019 | — | `in_progress` | **Slides: prompt-iteration problems & fixes decks (sorter + contracts specialist)** — 4 new docs in `docs/slides/`: 08-problems-sorter, 09-fixes-sorter, 10-problems-contracts-specialist, 11-fixes-contracts-specialist (same problems→fixes framing, per agent), README index updated. Docs-only, no changelog entry. | opencode (2026-08-15) | v0.18.0 | `docs/slides/README.md` index; changelog v0.15.0/v0.16.0/v0.17.0 iteration records |
...
| KANBAN-004 | [#3](https://github.com/Exios66/llm-entity-extraction/issues/3) | `in_progress` | **Extraction next arm — key_obligations span residual (v27 candidate)** — diagnostic done (2026-08-15): classified every miss on the 50-doc + sample5 surfaces. Dominant root cause = WRONG-SPAN at sentence level inside multi-requirement family sections (model emits ONE sentence per section; GT holds 3–10 requirement sentences: Ritter insurance/audit, Buffalo ROFR/insurance, NOVO, Goosehead; ~60–70% of misses are NEAR 0.35–0.59). Truncation confound documented: sample5 runs are `chunked=false` (Phasebio 0.125 vs 0.94 chunked@50 — pipeline, not prompt). v27 = v26 + multi-item family-section enumeration rule. Runs reserved: `qwen3.7-flash_contracts_specialist_v27_sample5`, `qwen3.7-flash_contracts_specialist_v26_sample5_chunked`, `qwen3.7-flash_contracts_specialist_v27_sample5_chunked`. | prompt-engineer (2026-08-15) | v0.18.0 | `reports/experiment_log.jsonl` v22/v23@50 + v26 sample5 rows; sim-matrix analysis (NEAR band) |
