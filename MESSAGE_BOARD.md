...
|---|---|---|---|---|---|---|
| KANBAN-049 | [#17](https://github.com/Exios66/llm-entity-extraction/issues/17) | `in_progress` | **Monte Carlo simulations folded into the GEPA prompt-iteration loop as a champion-contender step (human directive)** — the KANBAN-048 simulation suite becomes a formal GEPA selection layer: corpus-wide paired-bootstrap prompt ablation selects the **MC champion contender** per task (P(win) + CI-excludes-zero noise-floor contract + committee-voting robustness @ K), complementing same-surface A/B. **Half-corpus sample pilot first** (`--sample 0.5`, seeded): evaluate effectiveness/sample-efficiency of the MC selection (does half the shared surface recover the full/known champion? P(win) separation vs doc count) before full adoption. New `scripts/reporting/monte_carlo_gepa.py` + tests + memo; prompt-engineer agent documents the phase. | opencode (2026-08-16) | v0.19.0 | issue #17 (close on landing); `src/monte_carlo.py`; `monte_carlo_{prompt_ablation,ensemble}.py`; corpus `reports/monte_carlo/corpus.jsonl` |
