...
| KANBAN-025 | — (board-only) | `in_progress` | **HEARSAY prompt-iteration series** — step 1 SHIPPED (v0 baseline @5 = 1.0, saturates train). **Arm 2 in progress: test-set iteration surface** — sync the official 94-row test split (`nguha/legalbench` HF, No 53 / Yes 41, 5 slices) via the streamer's new `--test` flag → `mailroom-lb-hearsay-test`; v0 baseline @94 + diagnose → mutate (`legalbench_task_v1`) → same-surface A/B @94 with bootstrap-CI. Runs reserved: `qwen3.7-flash_legalbench_task_v0_test`(+`_classification_langfuse_test`) and `qwen3.7-flash_legalbench_task_v1_test`(+`_classification_langfuse_test`). I/O clean: LegalBench base_prompt inputs, single-token valid-class outputs. | opencode (2026-08-15) | v0.19.0 | `reports/experiment_log.jsonl` baseline records; KANBAN-022 wiring commit `f417227` |
