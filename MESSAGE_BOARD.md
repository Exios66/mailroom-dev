...
|---|---|---|---|---|---|---|
| KANBAN-025 | — (board-only) | `in_progress` | **HEARSAY prompt-iteration series (baseline done)** — step 1 SHIPPED: `qwen3.7-flash_legalbench_task_v0_baseline` = **exact_match 1.0 (5/5)**, 3,585 tokens / ~$0.0003; Langfuse mirror `_classification_langfuse_baseline` 3,481 tokens, 5 traces verified. **Insight: v0 saturates the 5-row train surface — next arm must sync the 95-row test set (`test.tsv`) for A/B resolution.** Subsequent GEPA-style arms continue on this card. | opencode (2026-08-15) | v0.19.0 | `reports/experiment_log.jsonl` baseline records; KANBAN-022 wiring commit `f417227` |
...
| KANBAN-023 | — (board-only) | `backlog` | **Sorter v12+ banked-cluster arm** — three lessons banked from the KANBAN-013 close-out, all with verified counterfactuals at 509 (to be measured on the same 243-doc surface, fp fb9f939d, seed 42, or the full-509 surface ~$0.26/run): (1) **strategic_alliance title-wins** — 5 fails @509 (Iovance/Adaptimmune = rule-21 inversion "governance → collaboration", Intricon license, Giggles consulting, FTE service), 0-risk counterfactual (all 32 alliance-titled docs GT alliance); (2) **cooperation-title is collaboration** — 3 fails (China Recycling, SPI, STW "COOPERATION AGREEMENT" → JV/development), 12/15 cooperation docs already correct; (3) **rule-21 inversion mechanism** — the model cites rule numbers while contradicting their content (Microgenics, El Pollo, Iovance, STW): test whether rule-block restructuring or family-boundary rules reduce the inversion rate. Runs reserved (when claimed): `qwen3.7-flash_sorter_v12_subtype_langfuse` on the 243 surface. | unclaimed | v0.19.0 | `memos/sorter_v10_v11.md` §Interpretation 4; KANBAN-013 close-out |
| KANBAN-021 | — (board-only) | `blocked` | **GEPA scale-up + prompt-efficiency arm** — v31 (token-efficiency refactor, −8.0% = 2,679 chars, all operative constraints preserved, 349 tests green) REGISTERED; accuracy A/B BLOCKED by the OpenRouter **weekly key limit (403)** — v28@510 completed 217/509 rows, v31@510 0/509. **Unblocks:** key reset / new OpenRouter key, then resume: `--dataset mailroom-cuad-contracts-full --sample 510 --seed 42 --chunked --manifest data/manifests/v28_510_chunked.jsonl` (v28, resumes 217) + fresh v31 manifest. Full-corpus surface + costs proven (~$0.19/run). Memo `contracts_specialist_v31.md`. | prompt-engineer (2026-08-15) | v0.19.0 | `reports/experiment_log.jsonl` v28/v31@510 records; memo `contracts_specialist_v31.md` |
