...
|---|---|---|---|---|---|---|
| KANBAN-031 | — (board-only) | `done` | **Sorter v13 maintenance-title arm — AGGREGATE WIN (KANBAN-031)** — v13 = v12 + rule 29 MAINTENANCE TITLE WINS (title-wins doctrine mirror of rules 23/24/26/28), fixing the rule-13 INVERSION cluster (model quotes "financial-sense maintenance → other" backwards; SUNTRONCORP/WELLSFARGO/PRIMEENERGY → other + AtnInternational → service, 3/4 deterministic). **Clean full-509 A/B (509-row paired intersection, seed 42, temp 0.1): v13 0.9430 vs v12 rerun 0.9293 = +0.0137, bootstrap CI [+0.0020, +0.0255], P(Δ≤0)=0.0090 — OUTSIDE the ±0.006 identical-prompt noise band → NEW AGGREGATE CHAMPION.** Maintenance cell 30/34 → **34/34 (1.0)**; 8 recovered / 1 regressed (ImperialGarden = pre-existing rule-24 outsourcing variance flip, NOT rule 29). 0-risk counterfactual (34/34 maintenance-titled docs GT maintenance). First v13 run DEGRADED (93/509 connection-error defaults — runner except-clause marks them completed) → clean rerun `subtype_v13_509_clean.jsonl` before any claim. **Runner: `run_langfuse_subtype_eval.py` selects Phoenix tracing by default** (`PHOENIX_TRACING=enabled` → `PhoenixTracer`; log record `tracing_backend="phoenix"`). Memo `memos/sorter_v13.md`; test `test_sorter_v13_maintenance_title_wins`; 56 relevant tests green; log/site regen done. | prompt-engineer (2026-08-15) | v0.19.0 | `src/prompts.py` SORTER_PROMPT_V13; `tests/test_prompts.py`; `memos/sorter_v13.md`; `scripts/eval/run_langfuse_subtype_eval.py` Phoenix sink |
...
| KANBAN-026 | — (board-only) | `in_progress` | **HEARSAY prompt-iteration series** (renamed from KANBAN-025 — number collision with prompt-engineer's run-sink card, see discussion) — step 1 SHIPPED (v0 baseline @5 = 1.0, saturates train). **Arm 2: test-set surface** — official 94-row test split (`nguha/legalbench` HF, No 53 / Yes 41, 5 slices) via the streamer's `--test`/`--local-dump` (Braintrust dataset-row writes are billing-blocked, so local JSONL + `--task-dataset` is the eval path). **Arm 3: Langfuse dataset mirror + LangSmith sink** — `sync_langfuse_datasets.py` mirrors train+test into Langfuse datasets (llm-dojo); LANGSMITH_PROJECT retargeted. **Arm 4 (v1 iteration): `legalbench_task_v1` @94 = 0.8511 (80/94) vs v0 0.7766–0.7872** — directional win, not 5%-significant (P=0.0905). **Arm 5 (v2 iteration, IN FLIGHT): full-reasoning diagnostic on the 14 v1 failures shows 8 are RUNNER artifacts** (the `_answer_task` 512-token reasoning truncation + `reasoning_effort=none` retry degrades rows 21/30/44/79/82/85/86 that full reasoning answers correctly with the SAME v1 prompt — a runner fix, not a prompt issue, banked as the next iteration) **+ 6 genuine content failures (91 rule_contradiction, 74/78 operative-fact, 72 protest-sign purpose, 68 sticker boundary, 39 one-off). `legalbench_task_v2` = v1.replace(rule 6)**: purpose-first reading (issue phrase names X), ACT/STATE carve-out (identification act, defamation, grievance, knowledge-acquaintance → No), intent-plan guardrail (email plan → ownership stays Yes), contradiction repair (removes the harmful "I am aware of the conduct → knowledge" YES example). Runs reserved: `qwen3.7-flash_legalbench_task_v2_test` (candidate) + `qwen3.7-flash_legalbench_task_v1_test_rerun94` (noise-floor control) — same surface (94-row test JSONL, fp 40cfb513, temp 0.0, qwen3.7-flash), fresh manifests. | prompt-engineer (2026-08-15) | v0.19.0 | `reports/experiment_log.jsonl` v0+v1 @94 records; `src/prompts.py` LEGALBENCH_TASK_PROMPT_V1/V2; memo `memos/legalbench_task_v1.md` + `memos/legalbench_task_v2.md` (pending) |
