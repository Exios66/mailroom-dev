...
| KANBAN-046 | — (board-only) | `in_progress` | **Monte Carlo simulation suite — ported from the RVL-CDIP-classifier (issue #17)** — zero-spend what-if analysis over the joint corpus (experiment log + manifests), mirroring the RVL-CDIP-classifier Posit Cloud Monte Carlo section: (1) `src/monte_carlo.py` shared helpers (normalize_dist / shannon_entropy / majority_margin / draw_committee / bootstrap / paired_delta_bootstrap / uncertainty_phrases / confidence_score / save_figure / style_axis), label vocabularies per task from `agents/sorter_agent.py`; (2) `scripts/reporting/monte_carlo_corpus.py` — joint corpus (experiment-log per-row results + manifests) → `reports/monte_carlo/corpus.jsonl` (gitignored, `--rebuild`) + `corpus-summary.md`; (3) `monte_carlo_ensemble.py` — accuracy(K) committee voting with bootstrap CIs, confidence blend (dominance/entropy/near-miss), escalation Pareto + `escalation_candidates.txt`; (4) `monte_carlo_prompt_ablation.py` — paired-bootstrap (model, A, B) deltas on shared docs (mean Δ / CI / P(win) / per-class / per-confusion); (5) `monte_carlo_failures.py` — retry/failover/fallback event simulation from n_error/status, 1K/25K/320K extrapolation + tail risk + max_tries×fallback sweep; (6) `monte_carlo_exemplars.py` — confusion-pair near-miss mining (correct reasoning naming the decoy) + Monte Carlo subset selection under token budget → exemplar appendix; (7) `monte_carlo_verify.py` — spend-minimal verification recipe printing run_langfuse_subtype/docclass eval commands (dry-run default). Tests `tests/test_monte_carlo.py` (network-free); memo `memos/monte_carlo_robustness.md`; docs (scripts/README, reports/README, .gitignore corpus). No model spend (plan-free). | opencode (2026-08-16) | v0.19.0 | `src/monte_carlo.py`; `scripts/reporting/monte_carlo_*.py`; `reports/monte_carlo/*`; `tests/test_monte_carlo.py`; memo; CHANGELOG `[Unreleased]` Added |
