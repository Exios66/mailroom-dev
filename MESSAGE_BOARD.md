...
| KANBAN-013 | [#11](https://github.com/Exios66/llm-entity-extraction/issues/11) | `in_progress` | **Sorter >0.93 tail-sampling iteration** — the v9 A/B left 18 fails, a 1-off long tail (no cluster >2); ~0.93 is the practical plateau on this corpus revision. 0.95 strict needs either tail-sampling iterations (per-error-class rules on the long tail) or a corpus re-baseline; proposal + data first (`V16_PROPOSITION.md` §18 risk register). **v10 iteration claimed: reflection found the plateau claim outdated — the tail is NOT uniform: marketing cell 0.5/10 (243) & 7/17 (509), unchanged since v6; strategic_alliance 5/32; cooperation-title 3/15. v10 = v9 + rule 26 MARKETING TITLE WINS (reward 7/risk 1/keep 10 @509; reward 5/risk 0 @243). Runs reserved: `qwen3.7-flash_sorter_v9_subtype_langfuse_rerun` (noise floor) + `qwen3.7-flash_sorter_v10_subtype_langfuse` (candidate), 250-stratified seed 42.** | prompt-engineer (2026-08-15) | v0.19.0 | `V16_PROPOSITION.md` §18; v9 run: qwen3.7-flash_sorter_v9_subtype_langfuse (strict 0.9259, 18 fails) |
