...
|---|---|---|---|---|---|---|
| KANBAN-033 | — (board-only) | `in_progress` | **MAUD + EDGAR S-1 corporate-record dataset wiring + new hierarchical sorter eval task** — (1) MAUD (Merger Agreement Understanding Dataset, CC BY 4.0) wired as a UTILIZED dataset: new `scripts/datasets/stream_maud_to_bt.py` (Zenodo `maud_v1.zip` primary / HF `theatticusproject/maud` mirror; 152 contracts + 25,827 train rows across 22 question families / 7 categories) → `mailroom-maud-contracts` (GT doc_type=merger_agreement + consideration-type subclass from MAUD GT: all_cash/all_stock/mixed_cash_stock/mixed_cash_stock_election) + `mailroom-maud-classification` (per-question multi-class rows, category/family as metadata) + `--local-dump` JSONL (Braintrust row uploads remain capped) + Langfuse dataset mirror; (2) NEW primary doc class `merger_agreement` in the sorter (7 classes) via new `sorter_docclass_v0` prompt + extended schema (doc_subclass for non-contract classes; the shared 6-class sorter surface is untouched — new classes only in the new prompt/runner); (3) NEW sorter eval task `run_langfuse_docclass_eval.py` scoring doc_type + subclass across a mixed corpus (CUAD contracts + MAUD merger agreements + S-1 corporate records); (4) EDGAR S-1 exhibit ingestion `scripts/datasets/stream_s1_exhibits.py` — SEC FTS search → filing index → corporate-record exhibits (EX-3.x/4.x/21.x/24.x/25.x) → text extraction → `mailroom-s1-corporate-records` (GT corporate_record + content-detected record_type subclass: bylaws/articles_of_incorporation/certificate_of_formation/powers_of_attorney/...; exhibit code kept as metadata). **Tertiary class level DROPPED per human directive (2026-08-15): only where the data necessitates it** — MAUD category + EDGAR exhibit code stay as dataset metadata, not classification dimensions. Pipeline mechanics verified live against EDGAR (FTS → index → EX-3.1/3.2/3.3 text extraction OK). Runs reserved (dry-run/pilot only): `qwen3.7-flash_sorter_docclass_v0_docclass_langfuse` (+ `_pilot`). | athena-database-agent (2026-08-15) | v0.19.0 | `scripts/datasets/stream_maud_to_bt.py`, `scripts/datasets/stream_s1_exhibits.py`, `scripts/eval/run_langfuse_docclass_eval.py`, `config/taxonomy.yaml`, `agents/sorter_agent.py`, `src/prompts.py` SORTER_DOCCLASS_PROMPT_V0 |
| KANBAN-032 | — (board-only) | `done` | **Sorter v14 marketing-title strengthening arm — LOGIC REPAIR, NOT a win; v13 stays champion** — v14 = v13 + rule 30 MARKETING TITLE WINS — STRENGTHENED (kills rule-26 narrowing: machinery re-reads, rule 9 hybrid read, first-named-family precedence). Full-509 A/B (Phoenix sink, seed 42, temp 0.1): **v14 0.9371 vs v13-clean 0.9430 = −0.0059, paired CI [−0.0177, +0.0059], P(Δ≤0)=0.8765 — INSIDE the ±0.006 band, NEGATIVE direction → NOT a claimed win.** What rule 30 DID do: marketing cell **14/17 → 16/17** (Audible + PACIRA recovered with rule-30 reasoning pinned; Zounds still resists even its own literal example — model-bound ceiling). **The flagged counterfactual FIRED: Playboy "CONTENT LICENSE, MARKETING AND SALES AGREEMENT" regressed license→marketing — carve-out (a) only cited the exact "Content License Agreement" phrase and did not generalize to license-PRIMARY titles → banked as the v15 lesson (widen the carve-out to any license-primary title).** 4/6 other regressions (LinkPlus affiliate, Liquidmetal development, Ehave reseller, HALITRON sponsorship) are families rule 30 never touches — run-to-run noise within the band. v13 stays aggregate champion; rule-30's directional marketing gain is banked. Memo `memos/sorter_v14.md`; CHANGELOG `[Unreleased]` Changed. | prompt-engineer (2026-08-15) | v0.19.0 | `src/prompts.py` SORTER_PROMPT_V14; `tests/test_prompts.py::test_sorter_v14_marketing_title_wins_strengthened` || KANBAN-030 | — (board-only) | `done` | **Contract-specialist v1..v16 archived to `src/prompts_archive.py`** — prompt-file bloat cut for later editing agents: the pre-documentation lineage (full-text v1..v7 + the early replace chain, ~1,000 lines / ~72 KB) moved into a FROZEN archive module and imported back into `src/prompts.py`, so the file drops **227 KB → 155 KB (−32%)** while EVERY version key stays resolvable (`get_prompt`, `PROMPT_VERSIONS`, manifests, Langfuse prompt syncs) and all 32 prompt strings are byte-identical (verified against git HEAD). Documented frontier lineage (v17..v32) with data-backed banners stays in `prompts.py`. Tests pin the archive rule (never edit an archived constant — a change = a new version key): `test_contracts_archive_preserves_identity_and_version_keys` + `test_contracts_archive_chain_heads_resolve` (384 tests green). | prompt-engineer (2026-08-15) | v0.19.0 | `src/prompts_archive.py`; `src/prompts.py` V1..V16 → import; CHANGELOG `[Unreleased]` Changed; commit `e0f758e` |
| KANBAN-029 | — (board-only) | `in_review` | **effective_date rule_contradiction repair (v32) — A/B COMPLETE: logic repair, v31 stays champion** — diagnosis: v12-era "defined term wins" tie-break contradicts CUAD GT (Agreement Date = answers[0] in 493/493); v32 = v31.replace(effective_date rule). **CLEAN full-510 A/B (495-row intersection): paired +0.0053, CI [−0.0052, +0.0159], P=0.1715 — INSIDE the ±0.011 noise band → NOT a claimed win; the first candidate run (52/509 transient errors) had inflated +0.0115 (survivorship bias) and was replaced by `..._510_full_clean`.** effective_date field +0.0171 (23 improved/11 regressed; 16/23 recoveries are the diagnosed target docs — rule-driven); v32 = effective_date field specialist on the frontier. **Never-null over-fire cluster deterministic (4/6 regressions reproduce: TRICITY null, ALLIANCEBANCORP blank-day 2006-11-01, SightLife 4/26, DYNTEK signature-over-effective-as-of) → banked as the v33 carve-out** (stated-FULL-date requirement). termination_clauses −0.0452 (10 docs 1.0→0.0) is run-to-run chunked variance in a field the rule does not touch. | prompt-engineer (2026-08-15) | v0.19.0 | `src/prompts.py` CONTRACTS_SPECIALIST_PROMPT_V32; `tests/test_prompts.py::test_contracts_v32_effective_date_convention_fix`; memo `memos/contracts_specialist_v32.md`; runs `..._510_full` (degraded) + `..._510_full_clean` |
...
| KANBAN-026 | — (board-only) | `in_progress` | **HEARSAY + CUAD-subtask LegalBench prompt-iteration series** (renamed from KANBAN-025 — number collision with prompt-engineer's run-sink card, see discussion) — step 1 SHIPPED (v0 baseline @5 = 1.0, saturates train). **Arm 2: test-set surface** — official 94-row test split (`nguha/legalbench` HF, No 53 / Yes 41, 5 slices) via the streamer's `--test`/`--local-dump`. **Arm 3: Langfuse dataset mirror + LangSmith sink** — `sync_langfuse_datasets.py`; LANGSMITH_PROJECT retargeted. **Arm 4 (v1): `legalbench_task_v1` @94 = 0.8511 (80/94) vs v0 0.7766–0.7872** — directional win, P=0.0905. **Arm 5 (v2): `legalbench_task_v2` @94 = 0.8830 (83/94) vs v1-control 0.8617 — Δ+2, paired CI [−4,+8], P=0.3345 → logic repair** (memo `legalbench_task_v2.md`). **Arm 6 (v3 prohibition rule): `legalbench_task_v3` 7/7 tasks 42/42 @42 = 100% vs v0 36/42** (memo `legalbench_task_v3.md`). **Arm 7 (v4 CUAD subtask series, COMPLETE): the 7 `legalbench_task_v3_<subtask>` keys were generic aliases; `LEGALBENCH_TASK_PROMPT_V4` = hygiene base (stray quote + rule-6 numbering collision fixed), `V4_CRE` = conditional-permission-carveout rule, `V4_CNTS` = conduct-restriction-covenant rule; 7 same-surface 6-row A/Bs (fp-matched, temp 0.1): CRE 0.8333→1.0 (deterministic row recovered), CNTS →1.0 (logic-repair grade), five subtasks unchanged at ceiling — memo `legalbench_task_v4.md`.** | prompt-engineer (2026-08-15) | v0.19.0 | `reports/experiment_log.jsonl` v0..v4 records; `src/prompts.py` LEGALBENCH_TASK_PROMPT_V0..V4/V4_CRE/V4_CNTS; memos `legalbench_task_v1/v2/v3/v4.md` |
| KANBAN-028 | — (board-only) | `done` | **Master ground-truth CSV added to the repo + repo-local default** — `data/cuad/master_clauses.csv` (510 CUAD contracts × 40 normalized `-Answer` columns, 3.95 MB) copied from the llm-mailroom checkout the repo previously depended on; `DEFAULT_MASTER_LABELS` now resolves to the repo-local copy first (env `MASTER_LABELS_CSV` still wins; sibling `../llm-mailroom/...` fallback retained); loader tolerant of the `Notice Period To Terminate Renewal- Answer` header variant so that category's answer loads. | prompt-engineer (2026-08-15) | v0.19.0 | `src/master_labels.py`, `data/cuad/master_clauses.csv` |
