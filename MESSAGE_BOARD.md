...
|---|---|---|---|---|---|---|
| KANBAN-044 | — (board-only) | `in_progress` | **llm-dojo-scoring integration — scoring/visualization/report library as a pinned git dependency (human request 2026-08-16)** — install `llm-dojo-scoring` (`Exios66/llm-dojo-scoring`, v0.1.0) as the single source of scoring/error-analysis/visualization/report code: (1) dep added to `pyproject.toml` + `requirements.txt` pinned to commit `29c192f` (no tags on the external repo yet); (2) new `src/dojo_config.py` maps `config/taxonomy.yaml` into the package `Settings` via `configure()` (embedding_enabled + cost_models list-form conversion + `load_env()` before the embedding fallback); (3) new `src/dojo_compat.py` holds the compat shims where the package contract differs (`get_field_types(taxonomy)`, `extraction_diagnostics(master=)` via `expected_resolver` + ported `_FIELD_CATEGORIES`, `build_scorers`/`scorer_names`/`cost`/`per_class_stats`/`macro_accuracy` object-list forms, docclass subclass scoping via `allowed=` sets, `classify_docclass_failure` None-on-ok); (4) the 6 local scoring modules become THIN RE-EXPORT SHIMS (`src/field_scoring.py`, `src/metrics.py`, `src/scorers.py`, `src/bootstrap.py`, `src/cost_models.py`, `src/experiment_log.py` core — renderers stay local) so llm-mailroom's `pip install -e .` imports keep working unchanged; `agents/sorter_agent.py` constants import from `llm_dojo_scoring.config`; `scripts/reporting/export_experiment_results.py` re-exports `llm_dojo_scoring.export`; `export_sweep_results.py` STAYS local (reference-format Notes/exact-prompt contract per KANBAN-040); (5) new dojo-* CLI capability (`dojo-analyze` markdown report + matplotlib plots, `dojo-export` canonical workbooks, `dojo-sync` live Langfuse/Phoenix trace→workbook) verified byte-identical vs `reports/sheets/` and documented; (6) tests: keep the 8 scoring test files (now exercising the shims) + new `tests/test_dojo_integration.py` (config wiring, shim parity pin, byte-identity, dojo-analyze smoke). Significant change → full suite + `release.py --check` gate. Follow-on card: llm-mailroom migration (KANBAN-005 territory). | opencode (2026-08-16) | v0.19.0 | `pyproject.toml`/`requirements.txt`; `src/dojo_config.py`; `src/dojo_compat.py`; 6 shimmed src modules; `tests/test_dojo_integration.py`; CHANGELOG `[Unreleased]` Added/Changed; memo `memos/llm_dojo_scoring_integration.md` |
| KANBAN-043 | — (board-only) | `in_progress` | **Champion-sweep extension — llama-4-scout + gpt-4o-mini on sorter_v13, full-509 (human directive)** — **† numbered KANBAN-043 after a collision: KANBAN-041 was already taken by the archived slides-deck card (origin lineage, 2026-08-16); reconciliation post on the discussion board.** Two full-509 subtype evals on the champion prompt sorter_v13, reasoning MEDIUM enabled, temp 0.1, `--research-funding-key`: (1) **`meta-llama/llama-4-scout`** ($0.10/$0.30 per M, 1.3M ctx — the Llama 4 family lightweight scout tier); (2) **`openai/gpt-4o-mini`** ($0.15/$0.60 — the GPT-4 mini tier). Companion to KANBAN-035 (gpt-5-nano 0.8978) + KANBAN-036 (deepseek-v4-flash, gpt-4.1-nano); champion baseline qwen3.7-flash 0.9430. Runs reserved: `llama-4-scout_sorter_v13_subtype_langfuse` + `gpt-4o-mini_sorter_v13_subtype_langfuse`; manifests `data/manifests/{llama4scout,gpt4omini}_sorter_v13_509.jsonl`. Smoke-tested on 1 doc (default key) before the funded launches. **Both runs landed in the log**: llama-4-scout 0.8880 (equiv 0.9077) @06:36, gpt-4o-mini 0.9312 (equiv 0.9352, exact 0.9961) @11:50 — see KANBAN-036 sweep table + discussion board; close-out pending. | prompt-engineer (2026-08-16) | v0.19.0 | `reports/experiment_log.{jsonl,md}` regen; frontier table update |
...
| KANBAN-036 | — (board-only) | `in_progress` | **Sorter subtype model sweep — 8 models on the champion `sorter_v13` prompt (full-509, seed 42, temp 0.1, reasoning medium, 0 errors) — COMPLETE, verified table** (strict subtype_accuracy / equiv / fails / strict-CI): (1) **`deepseek/deepseek-v4-pro` 0.9528** / 0.9548 / 24 / [0.9332, 0.9705] — the ONLY model to beat the qwen champion (+0.98pp); (2) **`qwen/qwen3.7-flash` 0.9430** / 0.9470 / 29 / [0.9214, 0.9627] — champion baseline; (3) **`openai/gpt-4o-mini` 0.9312** / 0.9352 / 35 / [0.9096, 0.9528] — KANBAN-043 arm (clean full-509 @11:50); (4) **`deepseek/deepseek-v4-flash` 0.9332** / 0.9352 / 34 / [0.9096, 0.9528]; (5) **`openai/gpt-5-nano` 0.8978** / 0.9018 / 52 / [0.8703, 0.9234]; (6) **`meta-llama/llama-3.3-70b-instruct` 0.8900** / 0.9116 / 56 / [0.8625, 0.9175]; (7) **`meta-llama/llama-4-scout` 0.8880** / 0.9077 / 57 / [0.8605, 0.9136]; (8) **`openai/gpt-4.1-nano` 0.8782** / 0.8959 / 62 / [0.8487, 0.9057]. **Cross-model significance NOT claimed** — the ±0.006 noise band was measured on identical-prompt qwen reruns, so a cross-model delta needs its own noise-floor control. **Duplicate-run caveats (verified against the log):** deepseek-v4-flash ran twice (0.9253 first @05:44 → 0.9332 clean rerun @06:38 — canonical 0.9332); llama-3.3-70b-instruct ran twice (0.8900 KANBAN-036 arm @06:54 → 0.8782 KANBAN-042 arm @10:51 — canonical 0.8900); llama-4-scout ran four times (0.8880 @06:36 KANBAN-043 — canonical, + 0.8762 @11:40/11:41/11:43 triple duplicate rerun); gpt-4.1-nano ran twice (0.8782 @06:27 KANBAN-036 + 0.8605 @11:59 resumed from a partial manifest to full-509 completion — the @11:59 run is the last full-509 completion). Runs + manifests in `data/manifests/` (gitignored); sweep arms = KANBAN-035/036/040/042/043. | opencode (2026-08-16) | v0.19.0 | `reports/experiment_log.{jsonl,md}`; `reports/sheets/Sorter_Model_Sweep_Results.xlsx` |
