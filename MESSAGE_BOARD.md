...
|---|---|---|---|---|---|---|
| KANBAN-047 | [#19](https://github.com/Exios66/llm-entity-extraction/issues/19) | `in_progress` | **Sync llm-dojo-scoring to encompass the additional document hierarchy (issue #19)** — extend the `llm-dojo-scoring` library beyond CUAD: **MAUD** (merger-agreement doc-class + consideration-type subclass + per-question classification), **LegalBench** (task-mode Yes/No/multiclass exact-match + per-class + CI), **chained evaluation runs** (sorter→extractor composite + handoff-scope ablations), **multi classification** (macro/micro per-class + confusion), **court opinions** (court_opinion doc-class path). New task-aware scoring modules + tests + docs in the nested repo (`src/llm-dojo-scoring/`), version bump + re-pin in this repo. | opencode (2026-08-16) | v0.19.0 | issue #19; `src/llm-dojo-scoring/`; `llm-dojo-scoring` GitHub repo |
| KANBAN-046 | [#18](https://github.com/Exios66/llm-entity-extraction/issues/18) | `in_progress` | **Full Phoenix local trace sink addition — documentation + solidification (issue #18)** — document the local Arize Phoenix trace sink (Langfuse-primary resolution, Phoenix fallback, OTel spans, local SQLite, discard-by-delete) and **cement the resume / checkpoint / queue / cache configurations** for cost efficiency: manifest resume + `--manifest` header contract (`src/evaluation.py::ManifestStore`), the annotation queue (`run_annotation_queue.py`), embedding/cache reuse in scoring, `--dry-run` + `assert_production_run` cost gates. Docs (AGENTS.md + wiki), audit + tests. | opencode (2026-08-16) | v0.19.0 | issue #18; `src/tracing.py`; `src/phoenix_tracing.py`; `src/evaluation.py` |
