...
| KANBAN-026 | — (board-only) | `in_review` | **HEARSAY prompt-iteration series** (renamed from KANBAN-025 — number collision with prompt-engineer's run-sink card, see discussion) — step 1 SHIPPED (v0 baseline @5 = 1.0, saturates train). **Arm 2: test-set surface** — official 94-row test split (`nguha/legalbench` HF, No 53 / Yes 41, 5 slices) via the streamer's `--test`/`--local-dump` (Braintrust dataset-row writes are billing-blocked, so local JSONL + `--task-dataset` is the eval path). **Arm 3: Langfuse dataset mirror + LangSmith sink** — `sync_langfuse_datasets.py` mirrors train+test into Langfuse datasets (llm-dojo); LANGSMITH_PROJECT retargeted. **Arm 4 (v1 iteration, prompt-engineer per human directive): `legalbench_task_v1` = v0 + hearsay-doctrine rule → @94 same-surface A/B 0.8511 (80/94) vs v0 0.7766–0.7872** — recovered 12 (all 10 deterministic v0 fails) / regressed 6, paired bootstrap CI [−0.0213, +0.1489] P=0.0905 (directional win, not 5%-significant). v2 lesson banked (non-verbal-conduct clause over-fire). Runs: `qwen3.7-flash_legalbench_task_v1_test` landed; `_classification_langfuse_test` pending. | opencode / prompt-engineer (2026-08-15) | v0.19.0 | `reports/experiment_log.jsonl` v0+v1 @94 records; `src/prompts.py` LEGALBENCH_TASK_PROMPT_V1; memo `memos/legalbench_task_v1.md`; commit `6e30481` |
