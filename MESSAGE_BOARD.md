...
|---|---|---|---|---|---|---|
| KANBAN-036 | — (board-only) | `in_progress` | **Sorter subtype model sweep — deepseek-v4-flash + gpt-4.1-nano + llama-4-scout on the champion prompt (full-509)** — per the human directive, full-509 subtype evals on sorter_v13 (champion), reasoning medium, temp 0.1, Phoenix sink, `--research-funding-key`: (1) **`deepseek/deepseek-v4-flash`** ($0.0629/M prompt + $0.1257/M completion; ~$0.46 est.); (2) **`openai/gpt-4.1-nano`** ($0.10/$0.40; ~$0.78 est.); (3) **`meta-llama/llama-4-scout`** ($0.10/$0.30, 1.31M ctx; smoke1 26.3k tok/1 doc, exact 1.0) — **QUEUED to launch AFTER the gpt-4.1-nano arm completes** (human directive). deepseek + gpt-4.1-nano full runs launched 2026-08-16 01:25; llama-4-scout smoke-tested 01:27. Runs reserved: `deepseek-v4-flash_sorter_v13_subtype_langfuse` + `gpt-4.1-nano_sorter_v13_subtype_langfuse` + `llama-4-scout_sorter_v13_subtype_langfuse`; manifests `data/manifests/{deepseekv4flash,gpt41nano,llama4scout}_sorter_v13_509.jsonl` (gitignored). Companion to KANBAN-035 (gpt-5-nano 0.8978 @509). | opencode (2026-08-16) | v0.19.0 | `reports/experiment_log.{jsonl,md}` regen; sorter_v13 |
