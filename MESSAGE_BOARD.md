...
|---|---|---|---|---|---|---|
| KANBAN-032 | — (board-only) | `in_progress` | **Sorter v14 marketing-title strengthening arm** — v13@509 (0.9430, 29 fails) leaves the marketing cell STUCK at 14/17 (0.8235) since v12 with **3 DETERMINISTIC fails across all runs (v9-clean/v12-orig/v12-rerun/v13-clean)**: Zounds "MANUFACTURING, DESIGN AND MARKETING AGREEMENT" -> manufacturing, PACIRA "STRATEGIC LICENSING, DISTRIBUTION AND MARKETING AGREEMENT" -> distributor, Audible "CO-BRANDING, MARKETING AND DISTRIBUTION AGREEMENT" -> co_branding. **Root cause = rule-26 NARROWING, proven by the model's own reasoning** — Zounds quotes rule 26 then defeats it ("a title naming marketing usually wins if the core is promotion; however, here the core is clearly production/manufacturing") even though rule 26's literal example IS that title; PACIRA applies rule 9's hybrid machinery read; Audible lets the first-named family win. Same inversion shape rules 28/29 fixed. **v14 = v13 + rule 30 MARKETING TITLE WINS — STRENGTHENED** (kills machinery re-reads + rule 9 hybrid read + first-named-family precedence; carve-outs (a) license-primary / operational-service and (b) rule 16 preserved). Counterfactual 0-score-risk (17/20 marketing-titled docs GT marketing; Playboy ×2 license-primary protected; HEMISPHERX GT supply already wrong either way — boundary noted in memo). Runs reserved: `qwen3.7-flash_sorter_v14_subtype_langfuse` (candidate) + `qwen3.7-flash_sorter_v13_subtype_langfuse_rerun_509` (noise-floor control) — full-509 (fp c2341957…, seed 42, temp 0.1, reasoning medium), fresh manifests, Phoenix sink. | prompt-engineer (2026-08-15) | v0.19.0 | `src/prompts.py` SORTER_PROMPT_V14; `tests/test_prompts.py::test_sorter_v14_marketing_title_wins_strengthened` |
