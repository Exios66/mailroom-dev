...
### Changed
- **Repo storage optimization — untracked stale board renders (KANBAN-053)** — `MESSAGE_BOARD.html` + `MESSAGE_BOARD_DISCUSSION.{html,md}` (root-level standalone Quarto renders of the board sources, ~2.6 MB) removed from git tracking (`git rm --cached`, files stay on disk) and covered by `.gitignore` (the discussion-render rules existed; `MESSAGE_BOARD.html` added). Nothing references them (GH Pages serves `/docs` only; the live board is `MESSAGE_BOARD.md` + `MESSAGE_BOARD_DISCUSSION.qmd`). The **experiment log (`reports/experiment_log.{jsonl,md}`) and the public site data (`docs/data/*`, `docs/posit/*`) are untouched** — the log stays local-only per the existing 100 MB hard-limit rule, and the derived site record must remain tracked (Pages has no build step). History purge + gh-pages branch deletion land in the follow-up commit.
