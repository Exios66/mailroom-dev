...
**2026-08-16 — opencode — KANBAN-036: model sweep COMPLETE — llama-3.3-70b-instruct landed, full results**

llama-3.3-70b-instruct full-509 run finished (01:52, 0 errors). Final sweep
table (full-509, sorter_v13, reasoning medium, temp 0.1, `--research-funding-key`):

| Model | strict | equiv | fails | tokens | ~cost |
|---|---|---|---|---|---|
| `qwen/qwen3.7-flash` (champion) | **0.9430** | — | — | — | — |
| `deepseek/deepseek-v4-flash` | **0.9332** | 0.9352 | 34 | 6.91M | $0.46 |
| `openai/gpt-5-nano` (KANBAN-035) | 0.8978 | 0.9018 | 52 | 6.94M | $0.48 |
| `meta-llama/llama-3.3-70b-instruct` | **0.8900** | 0.9116 | 56 | 6.66M | ~$0.70 |
| `meta-llama/llama-4-scout` | 0.8880 | 0.9077 | 57 | 6.67M | ~$1.50 |
| `openai/gpt-4.1-nano` | 0.8782 | 0.8959 | 62 | 6.65M | $0.78 |

**Verdict:** no cheap model matches the qwen champion; deepseek-v4-flash is the
standout budget arm (−0.0098 vs champion, far ahead of the rest), while the two
Llama arms (3.3-70b 0.8900 / 4-scout 0.8880) are a statistical tie inside the
±0.006 band. All are cost-floor frontier arms. Board card KANBAN-036 updated to
ALL-ARMS-COMPLETE; prompt-engineer has already banked the cross-model failure
traces into `sorter_v15` (KANBAN-038). Experiment log + site regenerated.

:::

::: {.entry data-date="2026-08-16" data-agent="prompt-engineer" data-card="KANBAN-038"}

**2026-08-16 — prompt-engineer — KANBAN-038 done (logic repair, NOT a win): `sorter_v15` A/B complete — v13 stays champion**

Full-509 A/B landed clean (seed 42, temp 0.1, reasoning medium, qwen/qwen3.7-flash,
Phoenix sink, 0 errors, `--research-funding-key`; run `qwen3.7-flash_sorter_v15_subtype_langfuse`,
manifest `data/manifests/sorter_v15_509.jsonl`). **Result: v15 0.9450 strict /
0.9509 equiv vs v13-clean 0.9430 / 0.9470 = +0.0020, paired bootstrap 95% CI
[−0.0059, +0.0098], P(Δ≤0)=0.4160 — INSIDE the ±0.006 identical-prompt noise
band → v15 is a LOGIC REPAIR, NOT a claimed win; v13 stays aggregate champion.**
Rule 31 recovered PACIRA (marketing — the deterministic cross-model fail named
verbatim in the carve-out: champion →distributor, gpt-5-nano →license,
gpt-4.1-nano →strategic_alliance, llama-4-scout →license) + 2 rule-24
outsourcing variance flips (NEXSTAR, ImperialGarden); regressed Paratek
(outsourcing→manufacturing, rule-24 variance) + Artara (license→development,
rules 19/21 as designed, equiv-recovered). The motivating license-primary
cluster was already ~handled on the champion: the one remaining "Content
License Agreement" fail (LejuHoldings "2") is a GT-labeling artifact — the
exhibit's actual title is "Mutual Termination Agreement" (rule 8 correctly
routes it `other`; rule 31 cannot legitimately override). **Banked: v15 = the
license-primary field specialist on the frontier.** Follow-on candidates for
v16 (each larger than license-primary ever was): development (6 champion fails
— Ritter/RevolutionMedicines/EmeraldHealth/ElPolloLoco/ArcaUsTreasury/Eton),
service (4 — IntegrityFunds/GpaqAcquisition/KUBIENT/FEDERATED), and
cooperation→collaboration (CHINARECYCLING on the champion; 10+ docs in the
weaker models). Memo [`docs/memos/sorter_v15.md`](docs/memos/sorter_v15.md) +
CHANGELOG `[Unreleased]` Changed entry + test
`test_sorter_v15_license_primary_title_wins` in this close-out. Card archived.

:::

::: {.entry data-date="2026-08-16" data-agent="prompt-engineer" data-card="KANBAN-038"}

**2026-08-16 — prompt-engineer — KANBAN-038 claimed: `sorter_v15` = license-primary (Content License) title-wins, from the cross-model v13 failure traces**

Claiming KANBAN-038 (board-only). **Run name reserved: `qwen3.7-flash_sorter_v15_subtype_langfuse`**
(full-509, seed 42, temp 0.1, reasoning medium, qwen/qwen3.7-flash, Phoenix sink,
`--research-funding-key`; manifest `data/manifests/sorter_v15_509.jsonl`).

**Diagnosis (cross-model, sorter_v13 champion surface):** the champion v13-clean
(0.9430/0.9470, 29 fails) and the four other-model sweeps (gpt-5-nano 0.8978,
gpt-4.1-nano 0.8782, llama-4-scout 0.8880, deepseek-v4-flash 0.9332) were
cross-referenced per doc. The universal license-primary cluster is the cleanest
mandated target: **LejuHoldings "Content License Agreement" -> `other` fails in
ALL FIVE models** (champion + 4 weaker), and Playboy / DataCall /
ChinaRealEstate / Ideanomics ×2 / Midwest / AlliedEsports / GluMobile (all
"Content License Agreement" titles) fail -> `ip`/`joint_venture`/`marketing`/
`manufacturing`/`other` in the weaker models. This is exactly the **banked v14
lesson** (widen rule 26 carve-out (a) to any license-PRIMARY title; Playboy
"CONTENT LICENSE, MARKETING AND SALES" must keep license).

**v15 = v13 (champion) + rule 31 LICENSE-PRIMARY TITLE WINS**: a "Content
License Agreement" (license as the primary family) stays license — never
`other`, never `ip`, regardless of co-named marketing/sales/distribution or an
IP-grant/joint-venture core. Carve-outs preserved: rule 13 (license+maintenance
...
marketing core -> marketing, e.g. PACIRA "Strategic Licensing, Distribution and
Marketing"). Follow-on cards spawned for the other universal clusters:
development (6 champ fails), service (4), cooperation->collaboration (cross-model
huge), outsourcing (v13 banked), and the 1-doc universal title families
(franchise/co_branding/distributor/hosting/supply/agency).

:::

::: {.entry data-date="2026-08-16" data-agent="opencode" data-card="KANBAN-036"}

**2026-08-16 — opencode — KANBAN-036: llama-3.3-70b-instruct arm launched; sweep results so far**

Human directive: run a **llama-3.3-70b** arm to accompany llama-4-scout, v13
prompt, full corpus, **instruct variant**. Slug verified: **`meta-llama/llama-3.3-70b-instruct`**
($0.10/M prompt + $0.32/M completion, 128K ctx). Smoke-tested 1 doc (default
key): exact 1.0, **154 completion tok/doc** (far leaner than scout's 1,894).
Full-509 run launched 01:43 (`--research-funding-key`, manifest
`data/manifests/llama33_70b_sorter_v13_509.jsonl`). **Sweep results so far
(strict subtype_accuracy @509, sorter_v13, reasoning medium, temp 0.1):**
`deepseek-v4-flash` **0.9332** (34 fails) · `gpt-4.1-nano` **0.8782** (62) ·
`llama-4-scout` **0.8880** (57) · llama-3.3-70b-instruct in flight. None
match the qwen champion 0.9430; all are cost-floor frontier arms alongside
gpt-5-nano 0.8978 (KANBAN-035).

:::

::: {.entry data-date="2026-08-16" data-agent="opencode" data-card="KANBAN-036"}

**2026-08-16 — opencode — KANBAN-036: llama-4-scout arm QUEUED after the gpt-4.1-nano arm (full-509, sorter_v13)**

Human directive: queue a **Llama 4 Scout** run on the same v13 full-corpus
subtype surface, sequenced **after** the OpenAI arm. Slug verified live against
the OpenRouter catalog: **`meta-llama/llama-4-scout`** (no `-instruct` suffix;
$0.10/M prompt + $0.30/M completion, 1.31M ctx). Smoke-tested 1 doc (default
key, Phoenix sink): **exact 1.0 / subtype 1.0, 26.3k tokens/doc** (1,894
completion — reasoning_effort=medium is token-heavy on this model, so the full
run lands ~$1.5 est.). Run reserved: **`llama-4-scout_sorter_v13_subtype_langfuse`**
+ manifest `data/manifests/llama4scout_sorter_v13_509.jsonl` — launch AFTER the
gpt-4.1-nano full run (`gpt-4.1-nano_sorter_v13_subtype_langfuse`) finishes:
both deepseek-v4-flash and gpt-4.1-nano full-509 runs are in flight right now
(launched 01:25, `--research-funding-key`), so the llama arm waits rather than
trampling. **Env gotcha found:** my shell exports a stale `BRAINTRUST_API_KEY`
(`sk-yp7…`, user axios337@gmail.com / UW-Madison) that overrides the correct
`sk-6r2…` in `config/environments/braintrust.env` and 403s on
`mailroom-cuad-contracts-full` — run with `env -u BRAINTRUST_API_KEY` (or
unset it) so the file key wins.

:::

::: {.entry data-date="2026-08-16" data-agent="opencode" data-card="KANBAN-037"}

**2026-08-16 — opencode — KANBAN-037 in_review: env files consolidated under config/environments/ + data placeholder folders**

Two-part board-only task delivered. **(1) Env routing fixed to
`config/environments/`:** the live dotenv files (`braintrust.env`, `.env`,
`langfuse.env`) and their `.example` templates now live under
[`config/environments/`](config/environments/) — `src/env_utils.py` exports
the shared path constants (`ENV_DIR` / `BRAINTRUST_ENV_FILE` / `DOTENV_FILE` /
`LANGFUSE_ENV_FILE`) plus `resolve_env_file()` (absolute paths pass through;
bare `--env-file` names resolve under `ENV_DIR`), and `load_env()` iterates
`(BRAINTRUST_ENV_FILE, DOTENV_FILE)` instead of repo-root paths.
`src/braintrust_config.py` + `src/langfuse_config.py` default to `None` →
resolved file and fall back to `DOTENV_FILE` (repo-root-absolute) instead of
CWD `Path(".env")`; `sync_langfuse_prompts.py` / `sync_langfuse_datasets.py` /
`run_annotation_queue.py` import the shared constant for their defaults.
`.gitignore` ignores the new paths (root entries kept one release as a safety
net); example headers + setup docs (README, AGENTS.md, wiki/Getting-Started,
config/README, src/README) updated to the new `cp` paths. **(2) Data
placeholder folders:** committed README placeholders for the wired-in data
under `data/` — `cuad_pdfs/`, `maud/`, `s1_corporate_records/`,
`legalbench_local/`, `manifests/`, `judgments/`, `samples/` (each gitignored
via `/*` + `!README.md` negation so the doc is tracked, the data is not).
**Verification:** `tests/test_env_utils.py` + `test_braintrust_config.py` +
`test_sync_langfuse_prompts.py` + `test_sync_langfuse_datasets.py` +
`test_langfuse_tracing.py` + `test_annotation_queue.py` = 54/54 green
(network-free). Two pre-existing env-dependent smoke failures unrelated to
this card: `test_langfuse_subtype_loop_wiring` (Phoenix default from
KANBAN-031 — passes with `PHOENIX_TRACING=disabled`) and
`test_subtype_loop_no_braintrust_logging` (real `config/environments/.env`
carries `LANGSMITH_TRACING=true` — the documented `.env` reload issue).
