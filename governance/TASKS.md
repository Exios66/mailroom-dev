...
| HUB-019 | `in_progress` | **docclass-merged corpus hardening P0 — baseline freeze, dataset contract, identity/provenance schema, taxonomy parity gate** — human directive: implement the docclass-merged ingress/evaluation-corpus plan §85 (P0) per its §84A sequencing and §84B no-silent-completion contract. Freeze current v7 as `docclass-merged-v0.1-working` (pin HF revision + baseline audit report + machine-readable manifest), define the canonical dataset contract (`DOCCLASS_CONTRACT.md` incl. §81 mapping table + merger_agreement→contracts_specialist semantics), establish stable `document_id` + source provenance + content hashes in the export path, add §63 contract tests (corpus-eda gains a suite), land the §65A taxonomy-parity gate (`scripts/taxonomy_parity.py` + board-governance CI), correct stale taxonomy language (five-class is complete — no extended taxonomy). P1–P5 remain queued behind their §84A dependencies. No ad-hoc HF upload code (§44A); no pushes in P0. | GLM-5.3-Flash (opencode) 2026-09-02 | — | plan §95 revision note (pasted by human 2026-09-02). CO-WORK SPLIT (human directive 2026-09-02, "work alongside, don't race"): first agent holds §85 freeze + contract + document_id/provenance/hashes + §63 tests (components 1–4); second agent takes the §65A taxonomy-parity gate (component 5, `scripts/taxonomy_parity.py` + board-governance CI) — disjoint surface, no dependency on the freeze. One card, two non-overlapping lanes; Evidence notes name which lane each commit serves. PARITY-GATE LANE LANDED (second agent): `scripts/taxonomy_parity.py` (strict equal on HUB_CLASSES/taxonomy.yaml live/ sorter vocab/specialist registry/v7-doc/dojo CORPUS_DOC_TYPES/entity PILOT universe/sandbox fixture; alias-aware structural rule for the documented compat surfaces — retired roster {compliance_filing, court_opinion, due_diligence} tolerated only as remnants per §60/v7-taxonomy §4; AST-parsed from literal sources, stdlib-only, network-free; 5 negative tests verified incl. anchor-edit propagation), CI step + taxonomy-surface path triggers in board-governance.yml, docs currency in AGENTS.md + README + wiki Board-Governance. Gate green on the live tree; board `check` green. Commits (swept/shared tree, co-work interleaved): 0bf22b6f, be1f56ef, ea76a22c, 8d72f5e1, 983a9b4b, 5c744b60. Discovered-not-delivered (release-train HUB-005 territory, NOT this card): dojo `mailroom.py` LIVE_DOC_TYPES/LIVE_SPECIALISTS and `config.py` LIVE_DOC_CLASS_KEYS are v0.5-era — still list compliance_filing as live and alias merger_agreement→contract; the gate's structural rule tolerates them, but renaming/realignment is a scoring-behavior change for the release train. Uncommitted residue: wiki Board-Governance.md doc-currency edit (second agent). |
