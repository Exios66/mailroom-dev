...
| HUB-050 | `in_progress` | **Watcher status notifications — 🔴 down alerts + 🟢 relaunch confirmations to the human's email** — human directive 2026-09-04 ("ensure a concise and stylized alert is sent to my email axios337@gmail.com when the watcher goes down, and send me a confirmation when it relaunches"; follow-up: "the status email should also not be firing excessively, only in emergency scenarios, and when the watcher is started or back up and running after a crash"). A dead process cannot email anyone, so: (1) the watcher writes an enriched periodic heartbeat (`watcher_heartbeat.json`, atomic temp+replace, pid/host/started_at/sha) + sends a 🟢 startup/relaunch confirmation at boot (daemon thread, fail-soft); (2) a separate lightweight watchdog process (`python -m pipeline.watchdog`) polls the heartbeat and sends the 🔴 DOWN alert (pid-dead fast path, laddered stale-confirmation for hung/sleep cases to avoid false positives) + 🟠 still-down reminders ONLY while an outage is active. **NO periodic healthy-status emails** (human anti-spam directive). SMTP reuses the gmail channel config + a test seam; knobs: `MAILROOM_STATUS_EMAIL` (default axios337@gmail.com), `MAILROOM_WATCHER_STATUS=0` kill-switch, `MAILROOM_WATCHDOG*` timing knobs. | opencode (GLM-5.3-Flash) 2026-09-04 | — | claimed at directive time; implementation + live down/up drill (real emails to axios337) to follow. **IMPLEMENTATION DONE 2026-09-04:** `pipeline/status_notify.py` (3 kinds ONLY — running/down/still_down, no digest; HTML+text renderers; SMTP factory seam `set_status_smtp_factory`; `MAILROOM_WATCHER_STATUS=0` kill-switch; every send fails soft), `pipeline/watchdog.py` (pure `evaluate()` transition core + `main()` loop; pid-dead/missing-heartbeat immediate 🔴, 2-stale ladder, 🟠 reminders only during an active outage, `MAILROOM_WATCHDOG=0` kill-switch, `MAILROOM_WATCHDOG_POLL_SECONDS=20`/`_STALE_SECONDS=30`/`_REMIND_MINUTES=60`), `bins.py:touch_watcher_heartbeat(extra)` (atomic enriched heartbeat: pid/host/started_at/git-sha), `watcher.py` hooks (enriched first + rescan heartbeats; 🟢 startup notice daemon after relations sweeper), conftest hermetics (`MAILROOM_WATCHER_STATUS=0` + `MAILROOM_WATCHDOG=0` + `MAILROOM_STATUS_EMAIL` popped). Tests: 13-case `test_status_notify.py` (renders, injected-factory send with MIME parsing, kill-switch, no-creds skip, soft failure, heartbeat roundtrip, watchdog transitions incl. recovery reset + startup notice + `start()` hook) — suites `test_status_notify` + `test_gmail_triage` + `test_gmail_intake` + dedup/reconcile/embedded/relations/specialists = 145 passed; Gmail mock smoke 10/10 PASS. Package AGENTS.md status-channel bullet added. PENDING: live down/up drill (real 🔴/🟠/🟢 emails to axios337) at watcher restart + watchdog bring-up. |
