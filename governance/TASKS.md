...
| HUB-059 | `in_progress` | **Connect the served Vercel board to the live board — live-only, no snapshot** — human directive 2026-09-06 ("connect the Vercel to the live board, not a snapshot"). The served dispatch board (https://mailroom-dev.vercel.app, `board-site/`) must always present the LIVE GitHub issues — never a hardcoded offline copy. The frontend (`index.html`) ships a large baked-in snapshot (`getRealData()`, ~660 lines copied from TASKS.md incl. a fake HUB-055 card) injected when the `/api/board` proxy is unreachable ("● OFFLINE · snapshot") — i.e. stale data shown as if it were the board. Deliverables: remove the snapshot fallback (the board = the GitHub issues, nothing else); on proxy failure show a clear live-only offline state (error banner + "no live connection"), never fake cards; redeploy `board-site/` to Vercel prod; verify `GET /api/board` serves the live cards + PATCH write-back; confirm git == production. (Unassigned triage lane + CORS + pagination + search-API lookup already landed in `f813734` by a parallel session.) | opencode 2026-09-06 | [#34](https://github.com/Exios66/mailroom-dev/issues/34) | claimed at directive time 2026-09-06; snapshot fallback removed from index.html (getRealData() deleted; refreshBoard catch now sets a live-only offline banner + "● OFFLINE · no live connection", never injects snapshot data); inline JS node --check passes. Redeploy + live verification to follow, then archive + close #34 in the same commit. |
