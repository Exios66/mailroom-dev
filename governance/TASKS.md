...
| HUB-038 | `in_progress` | **Intake agent v2 — triage + clean + prepare for the full pipeline** — human directive 2026-09-03 ("better formulate the main pipeline intake agent to triage, clean, prepare the data for classification and full pipeline processing, beyond its current state and mechanics, similar in nature to the free triage team, but applied to the intake agent responsible for primary text cleaning + processing + preparation"). Land in `packages/llm-mailroom`: upgrade `agents/intake.py` from the deterministic clerk (dojo `deterministic_normalize` re-export) to an LLM-assisted intake agent with (A) **advisory triage read** — port `GmailTriageAgent` mechanics (primary class / subclass / confidence / gist / keywords, vocabulary-clamped, own `intake_*` audit events, rides manifest + report + Gmail echo, NEVER overrules the sorter); (B) **gated LLM cleaning pass** — fused with the triage call; dojo `score_intake(method="llm")` scores it against the clerk gold; deterministic re-normalization after so `prep_invariants` hold; `normalize-intake` span keys byte-compatible (The-Mailroom mirror + HF pilots untouched); (C) **structure-aware preparation** — section index (headings + roles + char offsets, deterministically validated) driving the sorter's material-window selection (replaces blind HEAD+TAIL) plus capability/handoff flags shared with the triage-lane check. **Cost + efficiency mandate (human):** ONE fused intake LLM call per document, gated to messy / over-sorter-budget / scan-suspicious docs only (clean short docs stay all-deterministic); cheapest paid model `qwen3.7-flash` ($0.03/$0.13) — free tier stays the Gmail triage lane's privilege. Sorter is fed the intake prior via the user-message side (vendored `sorter_v14` prompt untouched — no lineage change). Prompt registry 16→17 (`mailroom-intake`). | opencode (GLM-5.3-Flash) 2026-09-03 | — | claimed 2026-09-03 by human directive; IMPLEMENTED 2026-09-03 — intake agent v2 with the NO-TRUNCATION doctrine (human directive: "stop the truncation of documents — use sliding windows to preserve document text so critical information is never missed"): `agents/intake.py` = deterministic clerk baseline + LLM-assisted pass (ONE fused call per window: TRIAGE advisory read via shared `validate_triage`, CLEAN structural repair re-normalized through the clerk, PREPARE section map with document-absolute validated offsets) gated to messy/over-budget docs (`MAILROOM_LLM_INTAKE=0` disables; fails soft); `sliding_windows` (paragraph-boundary, 15% overlap, full coverage — no window exceeds budget, nothing truncated); `agents/sorter.py` subclass bypasses the vendored HEAD+TAIL truncation (per-window classify + deterministic merge: plurality vote, mean confidence, first non-null subtype, joined reasoning; intake prior + retry preamble on EVERY window); graph wiring in `ingest_node` (gate, `intake-llm-prep` span, manifest `intake.triage`/`intake.prep` merge, state `intake_prep`) + `classify_node`/`retry_classify_node` (prior injection, full-text retry — removed the `[:12000]` slice); cheapest paid model `qwen3.7-flash` (free tier stays the triage lane); prompt registry 16→17 (`mailroom-intake`); conftest hermetic default off; docs currency (AGENTS.md, docs/agents.md §1+§8b, docs/configuration.md). Tests: new `test_intake_agent.py` (gate, validation, sliding-window coverage proofs incl. mid-document marker read, merged triage/offsets, sorter merge, prior/prefix per window, ingest wiring) + 16→17 invariants. Gates: llm-mailroom FULL suite 859 passed / 36 skipped / 0 failed; gmail smoke 10/10 PASS |
