...
- **HUB-067** (done 2026-09-09) — **Successor pointer to the standalone Digital-Mailroom repo** — human directive 2026-09-09 (org-migration audit, HUB-side leg). **DELIVERED (commit `0f06fb4`):** successor notes added to `README.md` (callout after the banner), `AGENTS.md` (Governance & task board section), and `governance/TASKS.md` (board header) pointing at `LLM-Mailroom-Services/Digital-Mailroom` and its DMR board (https://digital-mailroom-theta.vercel.app); this repo remains the canonical HUB-era board for its own scope. Companion leg: DMR-006 in the standalone repo (reference audit + wiring sweep + CI gate). Board gate: `board_state.py check` 0 errors / 0 warnings (10 open cards). Issue #48 closed 2026-09-09.
