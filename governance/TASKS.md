...
| HUB-050 | `in_progress` | **Watcher status notifications — 🔴 down alerts + 🟢 relaunch confirmations to the human's email** — human directive 2026-09-04 ("ensure a concise and stylized alert is sent to my email axios337@gmail.com when the watcher goes down, and send me a confirmation when it relaunches, ensuring I receive timely accurate status reports of the watcher; at the moment I have almost no idea when it is or is not successfully running"). A dead process cannot email anyone, so: (1) the watcher writes a periodic heartbeat file (`watcher_heartbeat.json`, atomic temp+replace) + sends a 🟢 startup/relaunch confirmation at boot (daemon thread, fail-soft); (2) a separate lightweight watchdog process (`python -m pipeline.watchdog`) polls the heartbeat and sends the 🔴 DOWN alert (pid-dead fast path, laddered stale-confirmation for hung/sleep cases to avoid false positives, still-down reminders, periodic 📊 healthy digest with uptime + today's terminal-stage counts + gmail last-poll age). SMTP reuses the gmail channel config + a test seam; knobs: `MAILROOM_STATUS_EMAIL` (default axios337@gmail.com), `MAILROOM_WATCHER_STATUS=0` kill-switch, `MAILROOM_WATCHDOG*` timing knobs. | opencode (GLM-5.3-Flash) 2026-09-04 | — | claimed at directive time; implementation + live down/up drill (real emails to axios337) to follow. |
