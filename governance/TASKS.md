...
| HUB-055 | `needs_attention` | **Wire the Kanban Dispatch Board into a served path — Vercel + GitHub-issues-backed, automatic site updates + editability** — human directive 2026-09-05 (issue #22: implement the nearly-composed `mailroom-dispatch-board.html` enhanced board, opened as new GitHub issues, served on Vercel or GH Pages with automatic site updates + editability). Landed per the approved plan (governance seeding → Vercel serve path → write-back → reverse-sync → docs): (1) **Card↔issue seeding** — every open card gains a synced GitHub issue (kanban label + lane/priority/attention/domain labels), Issue column links filled in this table; issue #22 = this card's mirror (relabeled). (2) **Vercel serve path** — `board-site/` (Vercel Root Directory) `api/board.js` (GET live cards from `labels=kanban` issues + POST new) + `api/board/[id].js` (write-back: lane = stage/* swap + dated comment, priority swap, body sections + assignees PATCH, archive=close/restore=reopen) + `lib/gh.js` (zero-dep tokenized proxy). (3) **`board-site/index.html`** — the composed dispatch board adapted to fetch `/api/board` (auto-updating; LIVE/OFFLINE badge), localStorage demoted to prefs + operator identity, every move/save/new-card emits write-through, delete→close. (4) **`board_state.py pull-issues`** — reverse-sync so TASKS.md stays canonical after served-board writes (drift report + `--apply` Lane + dated Evidence re-write; verified live by moving issue #27's lane and pulling it back cleanly). (5) Docs currency: AGENTS.md "Served Kanban board" section + command block, wiki Board-Governance.md mirror, CHANGELOG Unreleased. | opencode 2026-09-05 | [#22](https://github.com/Exios66/mailroom-dev/issues/22) | **decision:** deploy + `GITHUB_TOKEN` secret sit on the human's Vercel account — code is fully wired + locally verified (headless Chrome render: 10 live cards across lanes; PATCH round-trip exchanged stage/* + comment then restored; noop patch = zero churn; 405 guard) but not yet pushed to Vercel. Deploy: create a Vercel project with Root Directory `board-site/`, add secret `GITHUB_TOKEN` (fine-grained, Exios66/mailroom-dev Issues r/w + Metadata read), `vercel deploy` (or Import → Exios66/mailroom-dev → Root Directory board-site/). Seeded issues #23–#31 + relabeled #22; release-chain gate red pre-existing → spawned HUB-056 |
...
| HUB-057 | `in_progress` | **Cut official hub releases v0.3.0 + v0.4.0 with full release notes** — human directive 2026-09-05 (after the HUB-056 v0.4.0-tag retarget): the hub accumulated 84 commits of real work since v0.2.0 with NO hub release, and the HUB-054 session had attempted a hub v0.4.0 that was (correctly) retargeted away as an accidental The-Mailroom stamp. Cut BOTH official full hub releases through `release_chain.py`: **v0.3.0** = the triage/relations/corpus-samples epoch since v0.2.0 (HUB-039 free-swarm failover + capability-400 rotation + triage I/O debug capture, HUB-040 relations agent/ledger/KGs, HUB-041 insurance-subclass alignment + taxonomy-parity layer, HUB-043 intake-provenance dedup, HUB-046 claims-data-eda PDF samples, HUB-047 Enron Markdown samples, HUB-048 triage key-entity extraction, HUB-049 unknown-class fallback + review parking, HUB-050 watcher status alerts, HUB-051 relations production-readiness, HUB-052 relations mode toggle, HUB-053 mailroom-corpus HF loader + Gmail pilot notebook, HUB-005 third propagate sweep, ingest/intake consolidation tail, echo-depth + vendored free-only guardrail + atomic state writes); **v0.4.0** = the visualizer/dispatch-board epoch (HUB-054 terminal-stylized TUI + terminal GH Pages site, HUB-055 served Kanban dispatch board + board↔issue seeding + pull-issues reverse-sync, HUB-056 release-chain hygiene). Both get a real `## [0.3.0]` / `## [0.4.0]` CHANGELOG section, a pyproject bump, an annotated `mailroom-hub vX.Y.Z` tag, and a GitHub Release cut from the section. Docs currency: CHANGELOG, README scoring section (already HUB-054'd), AGENTS.md release-notes mention. | opencode 2026-09-05 | [#32](https://github.com/Exios66/mailroom-dev/issues/32) | claimed at directive; CHANGELOG sections authored from commit evidence; cuts + releases follow. **v0.3.0 CUT** — changelog section + pyproject 0.2.0→0.3.0 + annotated tag v0.3.0 + GitHub Release cut from the section; pushed. **v0.4.0 CUT** — changelog section + pyproject 0.3.0→0.4.0 + annotated tag v0.4.0 + GitHub Release; pushed. |
