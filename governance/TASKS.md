...
| HUB-056 | `assigned` | **Hub release chain red (pre-existing v0.4.0 tag break) — `release_chain.py check` fails on origin/main** — discovered while closing HUB-055 (the board-governance CI gate runs the chain check on every `scripts/`/`governance/` push): tag `v0.4.0` (cut 2026-09-04 05:46 by the HUB-054 session, message "0.4.0 — HUB-054: terminal-stylized TUI…") exists but the hub CHANGELOG has no `## [0.4.0]` section (newest is `[0.2.0]`) and `pyproject.toml` still says `0.2.0` — a release cut outside the release-chain discipline (HUB-024). Gate output: `ERROR tag v0.4.0 has no matching CHANGELOG section`, `ERROR hub pyproject version 0.2.0 is behind the newest tag v0.4.0`, `WARN tag message outside the convention`. Fix options (human decides): (a) properly complete the 0.4.0 cut — `release_chain.py cut 0.4.0` after moving the accumulated `[Unreleased]` content into a `## [0.4.0]` section + `--apply --tag`-wire to the existing tag, OR (b) retarget the tag to 0.2.0-patch territory if 0.4.0 was an accident (hub `pyproject` should match); recognized upstream-feeder releases (llm-mailroom v0.6.0, The-Mailroom v0.4.0) belong to their own repos and are NOT this tag's content. | unclaimed | [#31](https://github.com/Exios66/mailroom-dev/issues/31) | spawned from HUB-055 close-out 2026-09-05 (release-chain gate blocks / is red on scripts+governance pushes) |
