...
| HUB-048 | `assigned` | **Gmail triage agent: key concise entity extraction for ALL document types (esp. short correspondence — the Enron samples) using the EXISTING EXTRACTION_SCHEMAS per-class models** — human directive 2026-09-04 ("Can we ensure the enron examples will be properly handled by the triage gmail agent swarm? They should be able to perform key, concise entity extraction on all document types, ESPECIALLY THE SHORT EXAMPLES LIKE CORRESPONDANCE" + "It should be the extraction schema that we already have implemented"). Extend `GmailTriageAgent.triage()` (free-model lane) to return BOTH the classification read AND per-class key-entity extraction driven by `schemas.documents.EXTRACTION_SCHEMAS[primary_doc_class]` (e.g. CorrespondenceExtraction.sender/recipient/communication_date/action_items/subject_matter; InsuranceClaimExtraction.claim_number/insurer/claimed_amount; ContractExtraction.parties/effective_date/governing_law; etc.). The extraction rides `intake_meta["triage"]["extraction"]` → manifest → completion echo (INTAKE TRIAGE section) so short docs like Enron emails get a concise entity answer without the paid specialists. Advisory, validated, fail-soft like the rest of the lane. Must NOT break the existing triage schema contract, the paid pipeline's classification, or the conftest hermetics; verify against the 25 Enron Markdown samples in `packages/Enron-Evaluation-Environment/samples/`. | unclaimed | — | opened 2026-09-04 (directive + "use the existing extraction schema" clarification) |
