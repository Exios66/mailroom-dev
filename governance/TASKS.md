...
| HUB-058 | `in_progress` | **Full wiki sync + Vercel served-board documentation + governance reconciliation** — human directive 2026-09-06 ("ensure the full wiki is synced and properly up to the current working state, plus including all of the documentation for the new Vercel site, including its integration into the agent instructions and governance workflow"). Scope: (1) NEW dedicated wiki page `docs/wiki/Served-Board.md` — full operational doc for the live dispatch board (`board-site/` layout, issue→card read contract + PATCH write-back contract, config/env secrets, redeploy workflow, `pull-issues`/`sync-issues` reconciliation); (2) fix content-stale wiki pages (Home key facts v7→v8 corpus / built count, Architecture layout + surfaces incl. `board-site/` + terminal GH Pages site, Releases deploy surfaces + actual `--notes-file` release-cut practice, FAQ canonical-dataset answer + served-board Q&A, Getting-Started live-board pointer, Sub-Package-Sync all-packages sweep one-liner + HUB-044 race caveat); (3) reconcile the governance law — TASKS.md §"Issues vs board" board-only carve-out retired in favor of the card↔issue law (every card needs a `kanban` synced issue to appear on the served board) + post-site-edit `pull-issues` obligation, matching AGENTS.md; AGENTS.md served-board section cross-refs the wiki page; CHANGELOG `[Unreleased]` note. | opencode 2026-09-06 | [#33](https://github.com/Exios66/mailroom-dev/issues/33) | claimed at card creation; issue #33 opened same session (hub_card template body mirrored here). Plan approved by human 2026-09-06 (dedicated Served-Board page + reconcile the law). Evidence + commit SHAs to follow at delivery. |
