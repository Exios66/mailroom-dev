...
| HUB-013 | `assigned` | **Corpus-EDA v7 reconciliation** — discovered during the disk-sweep session (2026-08-31): the standalone clone at `~/Downloads/Mailroom-Corpus-EDA` holds UNPUSHED v7 work absent from both the monorepo (still v6 rev2 @ `b39245a`) and GitHub origin — local `main` at `1b0cf28` with unpushed commits `953a5c0`/`e83250c`/`6d93993`/`f2a30b2` (issue #5: v7 intent-hydration machinery — cross-walk, Enron join, LLM labeler, provenance schema; v7 EDA rerun; AGENTS v7 verified facts) plus a local `monorepo-import` branch. Reconcile: bring v7 into the monorepo (develop here per AGENTS.md), decide the upstream-publish path (the HUB-008 "no republish" stance needs revisiting for v7 — otherwise the monorepo becomes v7's sole home), then advance the corpus sync cursor. **CRITICAL: do not delete the Downloads clone until this lands — it is the only copy of the v7 work.** | unclaimed | — | disk-sweep session 2026-08-31; clone state verified via `git log --branches --not --remotes` |
