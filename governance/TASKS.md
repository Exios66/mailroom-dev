...
| HUB-037 | `in_progress` | **Gmail intake channel — llmmailroom@gmail.com as a second intake route** — human directive ("apply the newly created mailroom agent Gmail account, which acts as another channel for the agent to intake and process documents; ensure the watcher is aligned with the capabilities and route of intake" + follow-ups: example insurance claims to smoke-test Gmail+watcher connectivity/awareness; register the app password in GitHub secret manager). Landed in `packages/llm-mailroom`: `pipeline/gmail_intake.py` (stdlib IMAP-SSL poller → SAME inbox + `/upload` meta-sidecar; `[M:<id>]` subject routing; `\Seen` + Message-ID state dedup; sender allowlist + size cap; `/health checks.gmail_intake`), poller embedded in `Watcher.start()` (watcher.lock stays the single intake authority), intake **awareness** (`DocumentManifest.intake` carries source gmail/upload + message_id/sender through all terminal manifests; live traces tag `source-gmail`) — which also fixed a latent `existing_file_failed` bug (`_infer_matter_id` lived only on `InboxHandler`; startup-scan/rescan claims on `Watcher` crashed), and `src/scripts/gmail_smoke_test.py` (example insurance claim from the committed FNOL fixtures; mock mode verified PASS end-to-end; `--real` SMTP+IMAP; `--llm real`). **Check reaction (human directive 3):** at watcher claim time the source email is reacted to with the `✅` Gmail label (IMAP `X-GM-LABELS` in RFC 3501 modified-UTF-7 — Gmail rejects raw UTF-8 label bytes and literals in that position, live-verified BAD; `&JwU-` decodes to ✅ in the UI; one reaction per Message-ID; best-effort daemon thread; `MAILROOM_GMAIL_REACTIONS=0` disables). **Watcher now LIVE** (PID 98553, `data/watcher.out` + `data/mailroom.log`): drained the mailbox first sweep — Google noise logged + skipped, the human's sample PDF (SouthernStarEnergy SB-2A exhibit) queued → claimed → parked `review/` (fail-safe: no `OPENROUTER_API_KEY` configured — awaiting the human to add it, then recover/reprocess), ✅ reaction applied live and code-path verified; mock smoke 6/6 PASS. Email-format contract documented (docs/configuration.md § Emailing the mailroom) after the human asked — no subject keyword exists; attachments-only + optional `[M:]` routing. **Completion echo (human directive 4):** every terminal manifest (archived/review/failed) of a Gmail-intake document dispatches `dispatch_intake_echo` → `send_intake_echo` replies ON THE SOURCE THREAD (`In-Reply-To`-threaded, To: original sender) with the completion report — status, classification+confidence, extraction `_report`+fields, archive entry (path+sha256) or failure/review reason, and the audit chain via `get_audit_chain`+`verify_chain` — dedup per `(doc_id, stage)`, retried on next terminal event, `MAILROOM_GMAIL_ECHOES=0` disables, SMTP via `MAILROOM_GMAIL_SMTP_HOST/PORT`; smoke now 7/7 PASS (echo leg verifies To/Subject/In-Reply-To). Credentials in gitignored `.env` ONLY; `GMAIL_APP_PASSWORD` + `GMAIL_ADDRESS` registered via `gh secret set` on Exios66/mailroom-dev AND Exios66/llm-mailroom (never in argv/git). | opencode (GLM-5.3-Flash) 2026-09-02 | — | in progress — mock smoke PASS (connectivity/route/watcher/awareness/classify/reaction all green); 21 gmail tests green; FULL llm-mailroom suite rerun pending |
