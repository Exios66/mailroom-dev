...
| HUB-039 | `in_progress` | **Gmail free-triage team — production-pilot readiness sweep** — human directive 2026-09-03 ("ensure the new triage free agent team is fully built out and ready for production pilot testing via the singular document uploads thru Gmail"). Scope: end-to-end readiness audit of the single-document Gmail triage lane (code, config, prompt registry, tests, smoke, watcher runtime, credentials posture) + bring-up of everything within agent power + an honest gap list for the human. | opencode (GLM-5.3-Flash) 2026-09-03 | — | claimed at session start; audit underway. **AUDIT PROGRESS (2026-09-03):** code/config/prompt registry all COMPLETE and committed — `gmail_triage` registered in `llm/prompts.py` (production alias + template), taxonomy agent block (`z-ai/glm-5.2:free`, `max_input_chars: 12000`, `max_tokens: 700`), capability pre-check wired on BOTH watcher claim paths, route stamping + echo + reaction-retry in place; gmail suites 62 passed on the current tree; watcher tests 16 passed; mock smoke 10/10 PASS (full triage-chain rehearsal incl. route stamping, triage-lane archive with `triage_*` audit, multi-doc full-pipeline drop, reactions, echoes). **Sender allowlist (human directive):** `MAILROOM_GMAIL_ALLOWED_SENDERS=exios4@gmail.com,jjburleson@wisc.edu,axios337@gmail.com,exios4@protonmail.com` set in `packages/llm-mailroom/.env` (gitignored); live `load_config()` parse asserts the exact 4-address set; case-insensitive match verified (`_sender_address` lowercases both sides); allowlist tests 2 passed; roster documented in `.env.example` (expansion later per human). **Gaps found (readiness verdict: code-ready, runtime pending):** (1) `OPENROUTER_API_KEY` ABSENT from `.env` (only the 3 Gmail keys present) — the free model still needs an OpenRouter account key; without it the triage lane fails soft and single-doc uploads park to `failed/`; HUMAN must add the key. (2) Watcher NOT running (cleanly stopped 2026-09-02 22:42 after DNS `gaierror` flake; stale `watcher.lock` file harmless — `fcntl.flock` released on process death) — restart REQUIRED anyway to load the triage lane + HUB-038 changes (config is `lru_cache`d; long-running process). (3) LANGFUSE keys absent — NON-blocking (managed-prompt falls back to the identical in-repo template; local Phoenix tracing fallback; prompt sync optional post-key). **GAPS CLOSED (2026-09-03, same session):** (1) **`MAILROOM_LLM_FREE_ONLY` pilot guardrail** landed (`get_llm` refuses non-free model resolution — cost_models 0.0 or OpenRouter `:free` suffix — BEFORE any client exists; provider-credential check confines the key to the OpenRouter free tier; opt-in env flag, DEFAULT OFF = full-production behavior byte-identical; 9 tests `test_llm_free_only.py`) — commit `1f8f6387`, docs currency `32811ee3` (.env.example / configuration.md / gmail-intake.md / CHANGELOG). (2) Human supplied `OPENROUTER_API_KEY`; `.env` now carries key + `MAILROOM_LLM_FREE_ONLY=1` + the 4-sender allowlist. **Live proofs:** guardrail blocks the paid sorter (`RuntimeError: … model 'qwen/qwen3.7-flash' is not free`) while `get_llm('gmail_triage')` resolves `z-ai/glm-5.2:free`; ONE REAL free-model triage call PASS (98.9s incl. free-tier upstream 429s absorbed by the retry backoff) — `insurance_claim`/`first_notice_of_loss`/0.98 + grounded gist + 6 keywords; OPERATIONAL INTEL: the `:free` tier is upstream-rate-limited, expect ~1–2min triage latency under 429 pressure (async lane — intake itself never blocks). (3) **Watcher LIVE** (PID 56480/56482, worker `6f29e745`, `data/watcher.out`): `gmail_poller_started` poll_seconds=60, first sweeps clean (no poll failures). **PILOT POSTURE (human-directed):** human smoke-tests one document at a time from the 4 allowlisted senders; multi-document emails' paid path is STRUCTURALLY blocked while the flag is set (their documents fail-soft park instead of spending); unsetting `MAILROOM_LLM_FREE_ONLY` in full production restores the paid pipeline for multi-doc/inbox-CLI uploads. VERDICT: the free triage team is production-pilot READY. **FREE-SWARM FAILOVER 2026-09-04 (human report: "processing failed due to rate limits on GLM 5.2, on such a short document like the insurance claim"):** the single-doc pilot hit OpenRouter's shared free-pool saturation — all 5 retry attempts burned on the SAME primary (`z-ai/glm-5.2:free`) and the doc parked to review (the HUB-049 LLM-failure hardening worked exactly as designed: review park + echo, failed bin empty). Fix: ordered FREE-MODEL FAILOVER in the retry ladder — `taxonomy.yaml: free_model_swarm:` (glm-5.2 → ling-3.0-flash-fin → nemotron-3.5-lightning → inkling → inkling-small → dots-3-note → laguna-s → laguna-xs → lfm-2.5, priority order, code-specialized cohere excluded); `llm/retry.py:retry_chat_completion` rotates `model` to the next entry on RateLimitError when the current model is free (`llm_free_failover` log event; every generation traces its ACTUAL model), and a request only parks when the WHOLE swarm is limited; paid models NEVER rotate (failover would silently change cost). Text-conversion question answered: the PDF was ALREADY transcribed to text before the LLM call (`pdf_fallback_text chars=1092`) — the 429s are upstream pool saturation, independent of payload size. `llm/client.py:is_free_model` extracted as the shared predicate (assert_free_model unchanged in behavior). Tests: 6-case `test_llm_free_swarm.py` (rotation, swarm-walk-then-park, paid-models-never-rotate, connection-errors-don't-rotate, predicate, taxonomy guard) — retry/free-only/triage suites 70 passed. **LIVE FIX CHAIN VERIFIED 2026-09-04 06:44Z (outpatient insurance claim, real emails to axios337):** iteration 2 exposed ling's REAL failure via the new debug capture — 400 `model: inclusionai/ling-3.0-flash-fin does not support feature: structured-outputs` (Novita upstream; a MODEL-capability 400, previously silently discarded → empty triage park). Fixes: (a) FULL triage I/O debug capture (every call writes complete system/user/response/result under `data/debug/triage/<UTC>_<file>/`; hard call failures too; `triage_llm_io`/`triage_parse_failed`/`triage_llm_call_failed` events; commit `fa7d0be8`), (b) capability-400 rotation in the retry ladder (`_is_model_capability_error`; generic 400s never rotate; commit `d14b049b`), (c) robust JSON recovery (markdown fences / prose-wrapped objects — `_parse_model_json`). **Iteration 3 = FULL GREEN PATH:** glm-5.2 429 → ling capability-400 → `nvidia/nemotron-3.5-lightning:free` SERVED the read — `triage_llm_io doc_class=insurance_claim confidence=0.95 parse_ok=True`; FULL entity extraction on the manifest (claim_number 542452281547033, policy_number 1577999C7C7AD236, insurer CMS Medicare, insured_party TORRES JOSHUA, claim_type outpatient Part B, date_of_loss 2009-04-06, coverage_determination APPROVED, intent, subject_matter, keywords); `triage_classified` + `triage_archived` audits; archived echo `gmail_echo_sent stage=archived to=axios337@gmail.com`. Suites at fix time: free-swarm/retry/triage/intake = 101 passed. **SESSION NOTE 2026-09-04 (Gmail-pipeline debug pass):** full-suite debugging exposed a flaky tmpdir teardown race — `_run_triage_lane` spawned the HUB-040 `relations-scan` daemon thread (defaults ON; conftest had only disabled *embeddings*), racing pytest's rmtree (`OSError: [Errno 66] Directory not empty`). Fixed in conftest (`MAILROOM_RELATIONS=0`, test_relations re-enables) — FULL suite now 890 passed / 36 skipped / 0 errors (first clean run). Gmail mock smoke 10/10 PASS on the current tree. **Watcher RESTARTED on current code** (worker `f0c3ebc4`, PID 96674/96676, 2026-09-04 04:43Z): stale process predated the echo-depth `39cb3ed8`, vendored free-only chokepoint `ec4e160c`, and reviewer-escalation-text changes; post-restart verified — `gmail_poller_started` (60s), `relations_sweeper_started` (300s), `schema_ready`, IMAP sweeps completing clean (state `updated_at` advancing, 10 message-ids tracked, zero poll errors). Pre-restart `intake_llm_failed`/`review_classify_failed` entries are the expected `MAILROOM_LLM_FREE_ONLY=1` soft-fails (docs park to review/failed with echo; paid path structurally blocked by design — documented posture). |
...
| HUB-044 | `assigned` | **sync_packages.py worktree-race hardening (discovered in the HUB-005 third sweep)** — `patch_push` copies the package subtree from the WORKTREE over `git ls-files` while the clean-tree guard (`assert_clean_tree`) runs only ONCE at script start: a file saved by a parallel session mid-run is silently swept into the upstream propagation commit (2026-09-04 incident: an uncommitted `gmail_intake.py` echo-HTML edit went upstream as llm-mailroom `e496f1d2` and had to be reverted at `29249758`). Fix options (either or both): (a) copy committed blobs (`git archive HEAD -- packages/<pkg>` / pkg_tree read) instead of worktree files so uncommitted work can never propagate; (b) re-assert cleanliness immediately before each package's worktree copy (per-package guard, fail that package not the whole run). Docs currency: record the sanctioned all-packages sweep one-liner (`push --all --patch`) + the never-run-with-in-flight-package-edits caveat in the root AGENTS.md sync section (done alongside this card's spawn). | unclaimed | — | spawned from the HUB-005 third-sweep correction 2026-09-04 |
| HUB-045 | `in_progress` | **claims-data-eda: real-sample PDF examples (human directive 2026-09-04)** — add `docs/examples/` to the claims-data-eda standalone repo: 8 real corpus documents (carrier/inpatient/outpatient/pde, 2 each) from `Lucius-Morningstar/mailroom-corpus` (`ground_truth_hardened.jsonl`, doc_text verbatim) rendered to faithful A4 PDFs via a new deterministic zero-dep `scripts/render_samples.py` (Courier, WinAnsi, byte-reproducible), plus `manifest.json` (record provenance + sha256) and `docs/examples/README.md`; guard tests in `tests/test_examples.py` (artifacts + writer, no network); prune-doctrine exception recorded (like HUB-008). Commit in the monorepo first, then propagate upstream via `scripts/sync_packages.py push` (never with in-flight parallel-session edits — HUB-044 law). | opencode (GLM-5.3-Flash) 2026-09-04 | — | claimed at directive time; samples rendered from the live corpus rows (950 insurance_claim available incl. v8 property/auto; sampled the four health strata native to this package), PDFs structurally validated, 39/39 tests green. NUMBERING NOTE 2026-09-04: root AGENTS.md cites this card as HUB-046 while the board row was HUB-045 — a same-minute dual-insertion collision with the Enron-samples card (now HUB-047); reconcile the AGENTS.md citation when closing.. DUPLICATE NOTE 2026-09-04 (board hygiene, cross-session): this open row duplicates the archived HUB-046 close (commit `3526cccf` — same claims-data-eda PDFs work); the parallel session that owns it should delete this open row at next touch (not this session's lane). |
| HUB-043 | `in_progress` | **Watcher filename-only dedup silently drops re-sent Gmail documents (live pilot bug)** — human report 2026-09-04 ("i just sent a new email, no new watcher response"): the email queued fine (`gmail_attachment_queued`, route=triage, sender axios337@gmail.com) but `Watcher._process` skips the inbox file EVERY SECOND with `file_skipped_already_processed` — `_is_already_processed` matches ANY terminal manifest by `original_filename` alone, so a legitimate NEW email (new Message-ID) carrying an already-processed filename is refused forever. Fix: intake-provenance-aware dedup — a file's `/upload` sidecar (`message_id`/`upload_id`) must MATCH the terminal manifest's intake provenance to count as processed; mismatched provenance = a NEW document that MUST process. | opencode (GLM-5.3-Flash) 2026-09-03 | — | claimed at report time; fix + test + watcher restart to follow. **RESTART DONE 2026-09-04 (debug pass):** watcher restarted on the current code — including the provenance-aware dedup (`8f50cc26`), the triage-lane relations dispatch (`ca7e8633`), and the HUB-043 coordination work (`39cb3ed8` escalation text + `ec4e160c` vendored guardrail) — verified sweeps then. A HUB-040 relations-daemon-flake was found in the same area and fixed in conftest (`MAILROOM_RELATIONS=0`), committing with the FULL suite green at 890 passed / 36 skipped / 0 errors. **HUB-005 sweep coordination 2026-09-04:** the all-packages sync accidentally swept your UNCOMMITTED `gmail_intake.py` echo-HTML work upstream (llm-mailroom `e496f1d2`) — reverted there at `29249758` (file restored to the monorepo-committed state); your local uncommitted work was never touched and remains yours; it lands upstream when you commit + the next sweep runs. Driver race spawned as HUB-044. |
| HUB-042 | `assigned` | **Frozen docclass prompt surfaces still teach the pre-v8 4-token insurance subclass set + docclass-pilot examples lack the v8 LOB strata** — spawned at HUB-041 close (append-only law: discovered-but-undelivered gets a card before the parent closes). HUB-041 fixed the CODE catalogs + added `sorter_mailroom_v0`/`sorter_mailroom_pilot_v0`, but these FROZEN prompt surfaces (never mutated after a run) still enumerate only carrier/pde/outpatient/inpatient: the vision twin (`sorter_docclass_vision_v0/v1` rule 40 short form), the `_PILOT_CONTEXT` injected into the pilot judge/arbiter/boss prompts (HUB-020's neighborhood — its class-set rewrite should absorb the subclass lines), and the `reviewer_docclass_pilot_v0` subclass section. Also `Lucius-Morningstar/docclass-pilot` (48 strata, v5-era `--examples` surface for `run_hf_pilot.py`) carries no property/auto example rows. Follow-up: next docclass iteration (GEPA loop) derives NEW mailroom-named keys covering these surfaces + an A/B of the HUB-041 sorter keys vs v7/pilot-v3 defaults on the v8 corpus (same-surface, seed-matched); examples dataset refresh is a corpus-eda/Hub decision. | unclaimed | — | spawned from HUB-041 close-out 2026-09-03 |
| HUB-051 | `in_progress` | **Relations clerk (HUB-040) production-readiness completion — the research/review clerk was archived code-complete but is a NO-OP on the live system** — human directive 2026-09-04 ("ensure the review clerk is fully finished & built out — fully operational, production tested, ready for the full LLM-mailroom pipeline"). Production audit found: (G1) the Gmail triage lane NEVER writes the `documents` catalog row (watcher `_run_triage_lane` audits/archives/echoes/dispatches the relations scan but no `_catalog_upsert`) — live evidence: 4 `triage_archived` docs on disk + audit, ZERO archived catalog rows, 65 sweeps all `scanned: 0`, 130 ledger entries, 0 edges/embeddings; `scan_document` skips every triage doc `not_in_catalog`; (G2) the LLM judgment pass (`agents/relations.py:RelationsAgent.judge`) is DEAD CODE — never called by `scan_document`, `llm_pass_enabled()`/`top_k_llm_candidates` unused, so even flipping `relations.llm: true` does nothing; (G3) the documented CLI `python -m pipeline.relations_scan` crashes (`ModuleNotFoundError` — module never created, `main()` stranded in `pipeline/relations.py`); (G4) live catalog hygiene: 7 stale `processing` rows from dead watcher epochs. Fix: triage-lane catalog upsert (stage/doc_type/subclass/confidence/sha256/triage extraction), LLM-pass wiring (sub-threshold ambiguous band → judge → `llm_asserted` edges, confidence-gated, ledger), CLI module, stale-row cleanup; production verification on the live DB (real sweep, ledger verify, graphs, context block) + FULL suite gate. | opencode (GLM-5.3-Flash) 2026-09-04 | — | claimed at directive time; audit complete (evidence above); spawned from the HUB-040 archive record per append-only law (discovered-but-undelivered). **PROGRESS (fixes landed, all gates green):** (G1) `_triage_catalog_upsert` in watcher.py on BOTH triage terminal paths (incl. the HUB-049 LLM-unavailable review park) + `storage/catalog.py:write_document_record` now persists `file_sha256` (was never written by any caller — pre-existing gap); (G2) `_llm_judgment_edges` in `pipeline/relations.py`: near-miss capture during the signal loop, top-K ranked, `RelationsAgent.judge`, `llm_confidence_gate` 0.55, `llm_asserted` edges on the same upsert+ledger path, scanner re-validates the agent output against its own proposed pairs (defense-in-depth); (G3) `pipeline/relations_scan.py` created; (G4) live catalog reconciled (4 archived rows backfilled from the archive sidecars incl. sha/extraction, 3 dead processing rows → failed+escalation_reason). **BONUS PRODUCTION BUG FOUND + FIXED:** the embedding cosine signal NEVER worked outside the test seam — the dojo's public `get_embedding_model()` returns the model NAME string, so `model.encode(...)` was a TypeError on every real run (`relations_embed_failed`); `_embed` now drives the dojo's shared `_EmbeddingMatcher` singleton (local+remote, 90s-bounded, fail-soft; taxonomy wiring via the observability.field_scoring import side effect). Tests: +6 relations (LLM-pass wiring/gate/off-by-default/fail-soft, CLI module, embedder real-path + degrade) +2 gmail_triage (catalog row + fail-soft) + mock-smoke `catalog:` check (11/11 PASS). Gates: llm-mailroom FULL **949 passed / 36 skipped / 0 errors**; smoke 11/11. **LIVE PRODUCTION VERIFICATION (2026-09-04, real DB):** catalog reconciled; 4 archived docs scanned → 7 edges (6 same_matter + 1 party_overlap — shared insurer CMS between the outpatient claims), embeddings 1536-dim stored, ledger chain **OK (189 entries)**, knowledge graphs rendered (matter/global JSON+GraphML+HTML+PNG), advisory RELATED context block live. **REAL LLM JUDGMENT PASS PROOF:** `relations.llm: true` temporarily — the judge fired through the full free-swarm failover (glm-5.2 429 → ling capability-400 → `nvidia/nemotron-3.5-lightning:free` served, 29s), returned confidence 0.30-0.35 judgments on the genuinely-borderline contract↔claim pairs — correctly REFUSED by the 0.55 gate (ledger untouched; the model's conservative read matches "coincidence is worse than silence"); positive path proven by the mock wiring test. Taxonomy restored to `relations.llm: false` (pilot posture — the flip is a production config decision). Watcher restart with the new code + board close to follow. |
