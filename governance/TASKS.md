...
| HUB-060 | `in_progress` | **Served Vercel board — production-ready audit + UI improvements + accurate agents/evidence/description** — human directive 2026-09-06 (audit the Vercel site, improve display/UI, make it fully production ready, and ensure agents know to keep it updated and reading the most recent state of the board, always) + three correctness fixes: (1) **agents must reflect the actual agent/persona/harness**, not the human's GitHub profile — the board reads issue assignees (repo owner → every card shows 'Exios66'); fix = carry the agent identity in a new `### Owner` body section synced from TASKS.md Owner, served board reads that. (2) **Evidence trace empty for viewing** — sync real Evidence from TASKS.md into the issue's `### Evidence plan` section. (3) **Empty descriptions** — root cause: the old body-section rewrite glued headings (`### Lane\ndone### Task`) and dropped sections; make the section parser/writer glue-proof + recover corrupted bodies. Plus UI/display improvements (freshness/last-updated, auto-refresh/polling for most-recent state, offline/loading/empty states, mobile + a11y), production-readiness, and docs (AGENTS.md + wiki Served-Board.md) so agents know the workflow. | opencode 2026-09-06 | [#35](https://github.com/Exios66/mailroom-dev/issues/35) | claimed at directive time 2026-09-06; JS section parser/writer rebuilt glue-proof in board-site/lib/gh.js (agentsFromIssue reads `### Owner`; POST/PATCH write it); board_state.py sync-issues now pushes Owner/Task/Evidence body sections from TASKS.md (--apply backfilled all 8 open cards); repaired corrupted bodies #22 (HUB-055) + #34 (HUB-059). Frontend UI/redeploy/verify/docs to follow. |
