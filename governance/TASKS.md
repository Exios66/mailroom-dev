...
| HUB-055 | `in_progress` | **Wire the Kanban Dispatch Board into a served path — Vercel + GitHub-issues-backed, automatic site updates + editability** — human directive 2026-09-05 (issue #22: implement the nearly-composed `mailroom-dispatch-board.html` enhanced board, opened as new GitHub issues, served on Vercel or GH Pages with automatic site updates + editability). Landed per the approved plan (governance seeding → Vercel serve path → write-back → reverse-sync → docs): (1) **Card↔issue seeding** — every open card gains a synced GitHub issue (kanban label + lane/priority/attention/domain labels), Issue column links filled in this table; issue #22 = this card's mirror (relabeled). (2) **Vercel serve path** — `vercel.json` + `api/board.js` (GET live cards from `labels=kanban` issues) + `api/board/[id].js` (write-back: lane moves = stage/* label swap + comment, edits = PATCH issue) + `api/_lib/gh.js` (tokenized proxy). (3) **`board-site/index.html`** — the composed dispatch board adapted to fetch `/api/board` instead of hardcoded `getRealData()`, localStorage demoted to UI state only, every save/move emits write-through. (4) **`board_state.py pull-issues`** — reverse-sync so TASKS.md stays canonical after board writes (drift report + `--apply` Lane/Evi re-write). (5) Docs currency: AGENTS.md governance section, CHANGELOG Unreleased. Served URL + `GITHUB_TOKEN` secret live on the human's Vercel account. | opencode 2026-09-05 | [#22](https://github.com/Exios66/mailroom-dev/issues/22) | plan approved by human 2026-09-05; seeded issues #23–#30 + relabeled #22; implementation underway; release-chain gate red pre-existing → spawned HUB-056 |
