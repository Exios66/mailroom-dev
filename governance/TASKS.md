...
| HUB-023 | `in_progress` | **Rename the HF dataset `Lucius-Morningstar/docclass-merged` → `Lucius-Morningstar/mailroom-corpus`** — human directive ("docclass was always just a placeholder"). Hub leg (lucius agent): `HfApi.move_repo` preserves git history — verify the pinned revision `bb57c5ad` still resolves at the new name, verify no name collision, no other repo mutations. Monorepo leg: update every dataset-id reference (code + docs + dataset cards + audit manifest + parity surfaces) while KEEPING (a) the docclass prompt-family version keys (experiment identity — append-only law), (b) code file/flag names (docclass is the eval-arm name, not the dataset name), (c) the committed plan text (historical directive; gets a dated addendum note, not a rewrite). §44 pin discipline: pin re-verified post-rename; §84B docs currency in the same commits. | GLM-5.3-Flash (opencode) 2026-09-02 | — | human directive 2026-09-02; §44A centralized path; HUB-022. LANDED: Hub leg via lucius agent (HF token user-supplied, inline-only, cached per convention — never printed/stored): move_repo 401'd (no token) → human supplied token → pre-flight (no collision, tip bb57c5ad, 1,659 files) → move → post-flight verified (new repo sha == bb57c5ad, pin resolves, file count intact, old id = Hub redirect). Monorepo leg: 57 files / 82 exact-id replacements + composed f-string ids (hf_corpora × llm-mailroom/agent-mailroom/The-Mailroom) + prose (docs, dataset cards, wiki, audit manifest) — historically preserved: internal slug + source-docclass-merged tag (trace-tag immutability; 'corpus'/'mailroom-corpus' aliases added), release marker docclass-merged-v0.1-working (version label), prompt-family keys, file names, plan text (addendum only), CHANGELOGs/governance archives, derived EDA/graph artifacts (canonical bytes, refresh at release train). Contract §6 rename record; §44 pin re-verified. Suites green (corpus-eda 15, llm-mailroom 40 incl. run_hf_pilot 26 after fixing the dataset param threading into _execute_run, The-Mailroom 4, sandbox 51, dojo, agent-mailroom); parity gate green. Evidence: 3ad5d47d/77a26d69/d064ff9b (swept P0-completeness legs), 30b1f76c. |
