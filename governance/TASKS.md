...
| HUB-040 | `in_progress` | **Relations agent — semantic linking + auditable relations ledger + knowledge graphs (human directive 2026-09-03)** — a deterministic-first, optionally-LLM association layer ("the mailroom's research clerk"): links associated topics/documents/matters across the archive (lawyer's research methodology), the FINAL pipeline stage via post-archive dispatch (daemon thread at archive/review/failed, echo pattern — 13-node graph untouched) PLUS a regular background archive sweep (watermark-incremental, embedded in the watcher). Own hash-chained relations ledger (`relation_log`, `__relations__` scope, verify_chain reuse) + `relation_edges` + `relation_embeddings` (compute-once vector cache — O(new + candidates), never O(n²)). Deterministic layer: same-matter, keyword Jaccard, party overlap, temporal boost, embedding cosine (dojo get/warm_embedding_model — free, local). LLM pass (top-k candidates, clamped closed vocabulary, `mailroom-relations` prompt) CODE-COMPLETE but OFF in pilot (`relations.llm: false`); flips on in production. Context injection: advisory RELATED block into sorter/specialist handoff + RELATED MATTERS/DOCUMENTS echo section (zero-shot longitudinal lift). Knowledge graphs: matter graphs (typed doc nodes + related-matter bridges), global matter graph (aggregated pair weights), doc ego-graphs; GraphJSON/GraphML (stdlib, always) + Plotly HTML/PNG (optional-dep graceful skip); `relations_graph_rendered` ledger events. Locked decisions: post-archive dispatch / deterministic-only pilot / agents+echo injection / all KG formats. | opencode (GLM-5.3-Flash) 2026-09-03 | — | claimed; implementation starting per the approved plan (storage → scanner → agent → hooks → KG → tests → docs) |
