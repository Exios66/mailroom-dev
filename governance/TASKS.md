...
> repo runs its own DMR board (https://digital-mailroom-theta.vercel.app) for
> org/standalone-scoped work; this board remains canonical for HUB-era scope
> here, and the two served boards cross-link both ways (HUB-066).
...
| HUB-067 | `in_progress` | **Successor pointer to the standalone Digital-Mailroom repo** — human directive 2026-09-09 (org-migration audit, HUB-side leg). Add a concise successor note to this repo's README, AGENTS.md, and `governance/TASKS.md` header pointing at the standalone successor `LLM-Mailroom-Services/Digital-Mailroom` and its DMR dispatch board (https://digital-mailroom-theta.vercel.app); this repo remains the canonical HUB-era board for its own scope. Companion leg: DMR-006 in the standalone repo. | opencode (deepseek-v4-flash-vision-exp) 2026-09-09 | [#48](https://github.com/Exios66/mailroom-dev/issues/48) | opened 2026-09-09 by human directive; successor notes added to README + AGENTS.md + this board header. |
...
- **HUB-066** (done 2026-09-09) — **Cross-board navigation tab — HUB ↔ DMR served boards (original-repo leg)** — human directive 2026-09-09. **DELIVERED (commit `3d57a60`):** `board-site/index.html` gains the `.board-tabs` board-switcher in the top bar — active `HUB Board` chip + `DMR Board ↗` link to the standalone Digital-Mailroom board (https://digital-mailroom-theta.vercel.app); meta description names the cross-link. Deployed by the project's Git integration (Root Directory `board-site`). **LIVE VERIFIED:** https://mailroom-dev.vercel.app serves the switcher markup (`board-tabs`, `DMR Board ↗` href), the DMR link resolves 200, `/api/board` 200. Companion leg: **DMR-005** in `LLM-Mailroom-Services/Digital-Mailroom` (issue #9). Board gate at close: `board_state.py check` 0 errors / 0 warnings. Issue #47 closed 2026-09-09.
