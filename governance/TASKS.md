...
| HUB-052 | `in_progress` | **Relations clerk mode toggle — simple CLI + API live/pilot switching (human directive 2026-09-04: "ensure the knobs to configure the toggle of live vs pilot mode are simple and easily adjusted through cli or something even smoother")** — the HUB-051 close note left `relations.llm` a manual one-line taxonomy edit + watcher restart; this card makes the toggle first-class: a `python -m pipeline.relations_mode` CLI (`status | pilot | live [--model X] [--restart-watcher]`) that edits taxonomy surgically (comments preserved), validates the judge model against the free-only guardrail, and can gracefully restart the standalone watcher; plus authenticated `GET/POST /api/relations/mode` on the FastAPI (the "smoother" path — in-process config-cache clear so the embedded watcher picks the flip up instantly, no restart). Scope: relations layer only — the global `MAILROOM_LLM_FREE_ONLY` pilot gate stays a .env decision reported by `status`, never flipped by the toggle. | opencode (GLM-5.3-Flash) 2026-09-04 | — | claimed at directive time; spawned from the HUB-051 close note |
