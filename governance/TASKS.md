...
| HUB-066 | `in_progress` | **Cross-board navigation tab — HUB ↔ DMR served boards (original-repo leg)** — human directive 2026-09-09: the original HUB board must link to the standalone Digital-Mailroom board and vice versa. This card covers the `Exios66/mailroom-dev` leg: `board-site/index.html` gains a `HUB Board · DMR Board ↗` switcher linking to https://digital-mailroom-theta.vercel.app; the inverse tab on the DMR board is DMR-005 in `LLM-Mailroom-Services/Digital-Mailroom`. Deploy via the project's Git integration (push to main) and verify the production alias. | opencode (deepseek-v4-flash-vision-exp) 2026-09-09 | [#47](https://github.com/Exios66/mailroom-dev/issues/47) | opened 2026-09-09 by human directive; issue opened first (card↔issue law). Gates at close: live https://mailroom-dev.vercel.app shows the DMR tab; link resolves 200; deploy Ready. |
