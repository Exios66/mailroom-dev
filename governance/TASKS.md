...
| HUB-037 | `in_progress` | **Gmail intake channel — llmmailroom@gmail.com as a second intake route** — human directive ("apply the newly created mailroom agent Gmail account, which acts as another channel for the agent to intake and process documents; ensure the watcher is aligned with the capabilities and route of intake"). Land in `packages/llm-mailroom`: an IMAP-SSL poller (stdlib, no new deps) that fetches unseen messages from the agent mailbox and drops accepted attachments into the SAME inbox bin the watcher drains, using the `/upload` `<file>.meta` sidecar convention (matter routing via subject `[M:<id>]` tag or configured default); watcher core loop unchanged. Credentials in gitignored `.env` only. | opencode (GLM-5.3-Flash) 2026-09-02 | — | work in flight — claimed same session as directive |
