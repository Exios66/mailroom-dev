...
| HUB-049 | `assigned` | **Gmail triage agent key-entity extraction gap at `unknown` class** — during HUB-048 the free-model lane's extraction clamp returns `{}` for `unknown` primary class; if the free model fails to classify a short Enron email as correspondence, no entities are extracted. Follow-up: a deterministic default (e.g. correspondence-shaped best-effort extraction for short header-bearing texts even when the class is unknown) + coverage test. | unclaimed | — | HUB-048 close-out 2026-09-04 (discovered-but-undelivered law) |
